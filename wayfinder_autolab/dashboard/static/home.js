...
function renderSummary(runs) {
  ...
  const cards = [
    ...
    {
      label: "Promoted Experiments",
      value: `${summary.promoted_count || 0}`,
      detail: "Promotions recorded inside the visible run set.",
    },
    {
      label: "Best Run",
        ...
        <article class="panel run-card">
          <div class="run-card-header">
            <div>
              ...
              <p class="run-card-meta">
                ${escapeHtml(run.agent_label || "unknown")} • ${escapeHtml(run.run_kind || "harness")}
          ...
          <div class="run-card-chart">
            <div class="run-card-chart-header">
              ...
              <span class="mono">${escapeHtml(run.best_candidate_hash || "n/a")}</span>
            </div>
            ${seriesSvg}
          </div>
          ...
          <div class="run-card-links">
            ...
            ${run.best_candidate_hash ? `<a class="table-link" href="/experiments/${encodeURIComponent(run.best_candidate_hash)}">Best Experiment</a>` : ""}
          </div>
        </article>
      `;
    })
...
function sparklineSvg(points, metricKey) {
  ...
  const markers = values
    .map((point, index) => {
      ...
      const fill = point.promoted ? "#ffffff" : point.passed ? "#4ade80" : "rgba(255,255,255,0.22)";
      const stroke = point.promoted ? "#4ade80" : "rgba(255,255,255,0.18)";
      const radius = point.promoted ? 4.2 : 3.2;
