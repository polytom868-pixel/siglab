...
from wayfinder_autolab.evaluator.events import (
    classify_pt_market_state,
    detect_pt_roll_events,
    summarize_pt_universe,
)
from wayfinder_autolab.feature_dsl import load_feature_spec, resolve_feature_frames
...
def _weighted_score(
    ...
    *,
    ...
    if not chosen:
        raise ValueError("Candidate did not reference any compiled features")
    ...
    *,
    ...
    return {
        "lifecycle_policy": {
            ...
            "roll_days_before_expiry": candidate.risk.roll_days_before_expiry,
            "min_days_to_expiry": candidate.universe.min_days_to_expiry,
            "max_days_to_expiry": candidate.universe.max_days_to_expiry,
        },
...
async def compile_candidate(
    ...
    family_spec = load_family_spec(settings.root_dir, candidate.track, candidate.family)
    ...
    if candidate.track == "directional_perps" and candidate.family in PAIR_TRADE_FAMILIES:
        ...
        positions = _build_pair_trade_positions(
            ...
            max_asset_weight=candidate.risk.max_asset_weight,
        ...
        return CompiledChild(
            ...
            metadata={
                ...
                "feature_hash": _feature_hash(candidate.features),
                "gross_target": pair_policy["gross_target"],
                "max_gross_target": pair_policy["max_gross_target"],
                "entry_abs_score": pair_policy["entry_abs_score"],
                "exit_abs_score": pair_policy["exit_abs_score"],
                ...
                "leverage_profile": (
                    "unlevered"
                    if candidate.family == "perp_pair_trade_unlevered"
                    else "levered"
                ),
    ...
    if candidate.track == "directional_perps" and execution_profile in PERP_EXECUTION_PROFILES:
        symbols = await provider.discover_perp_symbols(
    ...
    if candidate.family == "basis_spread":
        symbols = await provider.discover_perp_symbols(
            candidate.universe.basis_groups,
            limit=candidate.universe.max_symbols,
        )
        ...
        score = _weighted_score(feature_frames, candidate.features, feature_weights)
        signal_components = _weighted_component_frames(
            feature_frames,
            candidate.features,
            feature_weights,
        ...
        pair_positions = _build_pair_positions(
            score,
            selection_count=int(
                candidate.params.get("selection_count", defaults.get("selection_count", 2))
            ),
            gross_target=float(candidate.params.get("gross_target", defaults.get("gross_target", 1.0))),
            max_asset_weight=candidate.risk.max_asset_weight,
            regime_gate_mask=regime_gate_mask,
        )
        ...
        return CompiledChild(
            ...
            metadata={
                ...
                "gross_target": float(
                    candidate.params.get("gross_target", defaults.get("gross_target", 1.0))
                ),
                "regime_gates": regime_gate_metadata,
                "source": bundle["source"],
                "bundle_as_of": bundle.get("bundle_as_of"),
    ...
    if candidate.family == "stable_pt_ladder":
        ...
        if not histories:
            raise ValueError("No stable PT histories available for this candidate")
        ...
        score = _ensure_single_eligible_scores(
            _weighted_score(feature_frames, candidate.features, feature_weights),
            pt_state["eligible"],
        )
        positions = _build_ranked_positions(
            score,
            long_count=int(
                candidate.params.get("selection_count", defaults.get("selection_count", 3))
            ),
            short_count=0,
            gross_target=float(candidate.params.get("gross_target", defaults.get("gross_target", 0.9))),
            max_asset_weight=candidate.risk.max_asset_weight,
        ...
        return CompiledChild(
            ...
            metadata={
                ...
                "gross_target": float(
                    candidate.params.get("gross_target", defaults.get("gross_target", 0.9))
                ),
    ...
    if candidate.family == "pt_yield_rotation":
        ...
        if not histories:
            raise ValueError("No PT histories available for this candidate")
        ...
        pt_positions = _build_ranked_positions(
            ...
            short_count=0,
            gross_target=float(candidate.params.get("gross_target", defaults.get("gross_target", 0.8))),
            max_asset_weight=candidate.risk.max_asset_weight,
            require_positive_longs=True,
        )
        roll_events = detect_pt_roll_events(
        ...
        hedge_mode = str(candidate.params.get("hedge_mode", defaults.get("hedge_mode", "none"))).lower()
        hedge_ratio = float(candidate.params.get("hedge_ratio", defaults.get("hedge_ratio", 1.0)))
        hedge_symbols: list[str] = []
        source = "pendle_public"
        ...
        return CompiledChild(
            ...
            metadata={
                ...
                "gross_target": float(
                    candidate.params.get("gross_target", defaults.get("gross_target", 0.8))
                ),
                "source": source,
                "bundle_as_of": prices.index.max().isoformat() if not prices.empty else None,
                "asset_breadth": len(prices.columns),
    ...
    if candidate.family == "lending_carry_rotation":
        ...
        if lending_bundle["prices"].empty:
            raise ValueError("No lending histories available for this candidate")
