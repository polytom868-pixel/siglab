...
class PlaceholderScreen(Screen[Any]):
    ...
    def on_mount(self) -> None:
        self.query_one('#placeholder-text', Static).update(f'{self._screen_name} — Coming soon')
...
class HelpScreen(ModalScreen[None]):
    ...
    BINDINGS: ClassVar[list[Binding | tuple[str, str] | tuple[str, str, str]]] = [Binding('escape', 'dismiss', 'Close'), Binding('q', 'dismiss', 'Close'), Binding('question_mark', 'dismiss', 'Close')]
    GLOBAL_KEYBINDINGS: ClassVar[list[tuple[str, str]]] = [('1-4', 'Switch to screen'), ('q / Ctrl+Q / Ctrl+C', 'Quit application'), ('? / F1', 'Show this help'), ('k/j', 'Navigate lists'), ('Enter', 'Select / confirm'), ('Escape', 'Close dialog / go back'), ('r', 'Refresh current screen'), ('/', 'Focus search/filter')]
    SCREEN_KEYBINDINGS: ClassVar[dict[str, list[tuple[str, str]]]] = {'market': [('j/k', 'Navigate symbol list'), ('/', 'Search symbols'), ('Enter', 'Select symbol'), ('r', 'Refresh data')], 'paper': [('s', 'Set symbol'), ('b', 'Toggle buy/sell'), ('t', 'Toggle market/limit'), ('Q', 'Set quantity'), ('p', 'Set price'), ('Enter', 'Submit order'), ('n', 'New session'), ('c', 'Cancel order'), ('r', 'Refresh')], 'risk': [('r', 'Refresh data'), ('j/k', 'Scroll alerts'), ('f', 'Cycle alert filter')], 'evidence': [('/', 'Filter evidence'), ('Tab', 'Switch pane'), ('Enter', 'Run demo step'), ('n/p', 'Next/prev step'), ('a', 'Run all steps'), ('f', 'Filter by source')]}
    ...
    def compose(self) -> ComposeResult:
        yield Vertical(Static(self._title_text(), id='help-title'), *(self._render_binding(key, desc) for key, desc in self.GLOBAL_KEYBINDINGS), *self._render_screen_section(), Static(''), Static('Press Escape, q, or ? to close'), id='help-dialog')
...
class NavSidebar(Static):
    ...
    def _build_items() -> list[ListItem]:
        ...
        for idx, label, _screen_id in NAV_ITEMS:
            item = ListItem(Static(f'  {idx}  {label}', classes='nav-item'), id=f'nav-{_screen_id}')
            items.append(item)
        return items
    ...
    def highlight_item(self, screen_id: str) -> None:
        ...
        for i, (_, _, sid) in enumerate(NAV_ITEMS):
            if sid == screen_id:
                lv.index = i
                break
_BUILTIN_SCREENS: dict[str, Callable[[], Screen[Any]]] = {'market': MarketScreen, 'paper': PaperScreen, 'risk': RiskScreen, 'evidence': EvidenceScreen}
...
class SigLabTUI(App[None]):
    ...
    BINDINGS: ClassVar[list[Binding | tuple[str, str] | tuple[str, str, str]]] = [Binding('q', 'quit', 'Quit', show=True), Binding('ctrl+q', 'quit', 'Quit', show=False), Binding('ctrl+c', 'quit', 'Quit', show=False), Binding('?', 'show_help', 'Help', show=True), Binding('f1', 'show_help', 'Help', show=False), Binding('escape', 'go_back', 'Back', show=False), Binding('1', "go_to_screen('market')", 'Market', show=True), Binding('2', "go_to_screen('paper')", 'Paper', show=True), Binding('3', "go_to_screen('risk')", 'Risk', show=True), Binding('4', "go_to_screen('evidence')", 'Evidence', show=False)]
