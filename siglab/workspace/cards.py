...
def render_experiment_card(
    *,
    ...
    features = [str(feature) for feature in spec.get("features") or []]
    trade_style = str(dict(spec.get("params") or {}).get("trade_style") or "").strip()
    ...
    body = "\n".join(
        [
            f"# Experiment {row.get('spec_hash')}",
            "",
            f"Family: `{family}`",
            f"Hypothesis: {hypothesis or 'n/a'}",
            f"Trade style: `{trade_style or 'unspecified'}`",
            f"Universe: `{json.dumps(spec.get('universe') or {}, ensure_ascii=True, sort_keys=True)}`",
            "",
            "## Summary",
            ...
            f"- `deployment_score`: {trial.get('deployment_score', generalization.get('deployment_score'))}",
...
def render_experiment_view_card(
    *,
    ...
    body = "\n".join(
        [
            f"# {kind.title()} {row.get('spec_hash')}",
            "",
