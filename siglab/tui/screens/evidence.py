...
class EvidenceGraphWidget(Static):
    ...
    def _filtered_nodes(self) -> list[dict[str, Any]]:
        ...
        if self._filter_text:
            result = [n for n in result if self._filter_text in str(n.get('label', '')).lower()]
        return result
            ...
            if result is not None:
                ...
                if rc == 0 and stdout.strip():
                    try:
                        ...
                        if summary_parts:
                            result_line = Text(f'       → {', '.join(summary_parts)}', style=ACCENT_GREEN)
...
class EvidenceScreen(BaseScreen):
    ...
    def _update_status(self) -> None:
        ...
        self._update_status_text(f'  {node_count} nodes  {edge_count} edges{filter_text}  {conn}{hints}')
    ...
    async def _run_demo_step(self, step_data: dict[str, Any]) -> int:
        ...
        try:
            ...
            demo.set_step_result(step_num, {'returncode': returncode, 'stdout': stdout, 'stderr': stderr})
            return returncode
        except Exception as exc:
            demo.set_step_result(step_num, {'returncode': -1, 'stdout': '', 'stderr': str(exc)})
