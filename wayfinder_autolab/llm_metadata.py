...
SUPPORTED_LLM_PROVIDERS = frozenset({"kimi", "deepseek", "openrouter"})
...
def resolve_llm_thinking_mode(
    ...
    *,
    ...
    if resolved_provider == "kimi":
        return str(getattr(settings, "kimi_thinking", "") or "").strip().lower()
...
def resolve_llm_model(
    ...
    *,
    ...
    return str(getattr(settings, "kimi_model", "kimi-k2.5") or "kimi-k2.5")
...
def resolve_llm_api_key(settings: Any, *, provider: str | None = None) -> str | None:
    ...
    return getattr(settings, "kimi_api_key", None)
...
def resolve_llm_base_url(settings: Any, *, provider: str | None = None) -> str:
    ...
    return str(getattr(settings, "kimi_base_url", "https://api.moonshot.ai/v1"))
