...
class LLMRoutingPolicy:
    ...
    def candidates(self, *, provider: str, stage: str | None, primary: str) -> list[str]:
        if provider != 'bai':
            return [primary]
        configured = [primary, str(getattr(self.settings, 'bai_fallback_fast_model', '') or ''), str(getattr(self.settings, 'bai_fallback_reasoning_model', '') or ''), str(getattr(self.settings, 'bai_model', '') or '')]
        ordered: list[str] = []
        ...
        viable = [model for model in ordered if model not in self.health.unavailable and model not in self.health.quota_blocked and (not (latency_sensitive and model in self.health.latency_demoted))]
        if viable:
            return viable
        if not latency_sensitive:
            return []
        return [model for model in ordered if model not in self.health.unavailable and model not in self.health.quota_blocked and (model in self.health.latency_demoted)]
    ...
    def snapshot(self) -> dict[str, Any]:
        return {'unavailable': sorted(self.health.unavailable), 'quota_blocked': sorted(self.health.quota_blocked), 'latency_demoted': sorted(self.health.latency_demoted), 'recent_errors': dict(self.health.recent_errors)}
