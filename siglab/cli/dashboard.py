...
def add_subparser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    start_parser = subparsers.add_parser('dashboard-start', help='Start the FastAPI dashboard (default port from $PORT or 8080).')
    ...
    stop_parser = subparsers.add_parser('dashboard-stop', help='Stop the running FastAPI dashboard.')
    stop_parser.add_argument('--port', type=int, default=_DEFAULT_PORT, help=f'Port (default: {_DEFAULT_PORT})')
    compat_parser = subparsers.add_parser('dashboard', help='[Legacy] Start dashboard (same as dashboard-start).')
...
def run_dashboard_start(args: argparse.Namespace) -> None:
    ...
    uvicorn.run('siglab.dashboard.app:app', host=host, port=port, reload=reload, log_level='info')
...
def run_dashboard_stop(args: argparse.Namespace) -> None:
    ...
    port = int(args.port)
    try:
        result = subprocess.run(f'lsof -ti :{port}', shell=True, capture_output=True, text=True, timeout=5)
