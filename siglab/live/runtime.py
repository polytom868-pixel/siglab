...
def _check_live_enabled() -> None:
    ...
    if os.environ.get('SIGLAB_LIVE_ENABLED') != '1':
        raise RuntimeError('dry_run=False requires SIGLAB_LIVE_ENABLED=1. Set the environment variable to confirm live execution intent. This is a safety gate — see siglab/live/runtime.py.')
...
class SoDEXExecutionAdapter:
    ...
    def setup(self) -> dict[str, Any]:
        ...
        account_id_raw = signing.get('accountID') or signing.get('account_id') or self.config.get('sodex_account_id')
        ...
        self.client = SoDEXSignedPerpsClient(api_key_name=str(api_key_name), account_id=account_id, signer=signer, nonce_manager=nonce_manager, environment=str(environment), dry_run=bool(signing.get('dry_run', self.config.get('sodex_dry_run', True))))
    ...
    async def place_market_order(self, **kwargs: Any) -> tuple[bool, str]:
        ...
        order = perps_order_item(cl_ord_id=f'siglab_{uuid.uuid4().hex[:12]}', modifier=1, side=1 if is_buy else 2, order_type=2, time_in_force=1, quantity=str(size), reduce_only=reduce_only)
    ...
    def dependency_report(self) -> dict[str, Any]:
        ...
        required = {'get_user_state': bool(getattr(client, 'get_user_state', None) or getattr(client, 'get_state', None) or getattr(client, 'account_state', None)), 'update_leverage': bool(getattr(client, 'update_leverage', None) or getattr(client, 'update_leverage_request', None)), 'place_market_order': bool(getattr(client, 'new_order_request', None) and getattr(client, 'send_signed_request', None)), 'all_mids': bool(getattr(client, 'all_mids', None) or getattr(client, 'mark_prices', None) or getattr(client, 'tickers', None))}
        signed_ready = all([client is not None, signer is not None, bool(api_key_name), account_id is not None, bool(nonce_store), not [name for name, present in required.items() if not present]])
        ...
        return {'client_configured': client is not None, 'required_methods': required, 'missing_methods': [name for name, present in required.items() if not present], 'wallet_address_configured': bool(self.wallet_address), 'dry_run': getattr(client, 'dry_run', True) if client is not None else True, 'signed_path': {'ready': signed_ready, 'environment': str(environment), 'signer_available': signer is not None, 'signer_type': getattr(signer, 'signer_type', None) if signer is not None else None, 'accountID_present': account_id is not None, 'api_key_name_present': bool(api_key_name), 'nonce_store_ready': bool(nonce_store), 'missing_prerequisites': missing_signed}, 'rate_limit_scope': {'budget_per_minute': 1200, 'scope': 'per_ip', 'local_scheduler_only': True, 'operator_warning': "SigLab's built-in SoDEX weight scheduler is process-local. Use an external shared limiter when multiple processes share one egress IP."}}
...
def _compact_weights(payload: dict[str, float], *, epsilon: float=1e-09) -> dict[str, float]:
    return {str(symbol): round(float(weight), 6) for symbol, weight in payload.items() if abs(float(weight)) > epsilon}
...
class DirectionalPerpsSigLabStrategy(Strategy):
    ...
    async def setup(self) -> None:
        ...
        self.sodex_adapter = SoDEXExecutionAdapter(self.config if isinstance(self.config, dict) else {}, wallet_address=self._get_strategy_wallet_address())
    ...
    async def update(self) -> StatusTuple:
        ...
        if self._circuit_breaker.peak_equity <= 0.0 or account_value > self._circuit_breaker.peak_equity:
        ...
        peak_dd = (account_value - self._circuit_breaker.peak_equity) / self._circuit_breaker.peak_equity if self._circuit_breaker.peak_equity > 0 else 0.0
        ...
        for order in plan:
            ...
            await adapter.update_leverage(asset_id=asset_id, leverage=leverage, is_cross=True, address=address)
            ok, result = await adapter.place_market_order(asset_id=asset_id, is_buy=bool(order['is_buy']), slippage=_finite_float(runtime.get('slippage'), 0.0035), size=adapter.get_valid_order_size(asset_id, _finite_float(order['size'])), address=address, reduce_only=bool(order.get('reduce_only', False)))
            if not ok:
    ...
    async def withdraw(self, **kwargs: Any) -> StatusTuple:
        ...
        for symbol, qty in current_positions.items():
            ...
            if size <= 0:
                continue
            ok, result = await adapter.place_market_order(asset_id=asset_id, is_buy=_finite_float(qty) < 0.0, slippage=_finite_float(runtime.get('slippage'), 0.0035), size=size, address=address, reduce_only=True)
            if not ok:
    ...
    async def dependency_report(self) -> dict[str, Any]:
        ...
        return {'strategy_name': self.name, 'spec_hash': self.live_spec.get('spec_hash'), 'runtime': dict(self.live_spec.get('runtime') or {}), 'sodex_adapter': adapter_report}
    ...
    async def _build_live_status(self, *, include_trade_plan: bool) -> StatusDict:
        ...
        trade_plan = self._build_trade_plan(target_weights=dict(target_snapshot['target_weights']), current_positions=current_positions, mids=dict(target_snapshot['mid_prices']), account_value=account_value, leverage=leverage, min_trade_usd=_finite_float(runtime.get('min_trade_usd'), 25.0), circuit_breaker=self._circuit_breaker) if include_trade_plan else []
        return {'portfolio_value': account_value, 'net_deposit': net_deposit, 'gas_available': 0.0, 'gassed_up': True, 'strategy_status': {'spec_hash': self.live_spec.get('spec_hash'), 'strategy_name': self.live_spec.get('strategy_name'), 'family': self.live_spec.get('family'), 'source': target_snapshot.get('source'), 'bundle_as_of': target_snapshot.get('bundle_as_of'), 'latest_signal_timestamp': target_snapshot.get('timestamp'), 'dry_run': self._adapter_dry_run(), 'current_account_value': round(account_value, 6), 'target_weights': _compact_weights(dict(target_snapshot['target_weights'])), 'current_positions': _compact_weights(current_positions), 'trade_plan': trade_plan, 'compiled_metadata': target_snapshot.get('compiled_metadata')}}
    ...
    async def _latest_target_snapshot(self) -> dict[str, Any]:
        ...
        runtime = dict(self.live_spec.get('runtime') or {})
        provider = MarketDataProvider(settings, ParquetLake(settings.data_lake_dir), config_path=runtime.get('sosovalue_config_path'))
        try:
            ...
            targets = compiled.target_positions.reindex(prices.index).ffill().fillna(0.0).shift(1).fillna(0.0)
            if targets.empty:
                raise ValueError('No target history available for deployd strategy')
            timestamp = prices.index[-1]
            return {'timestamp': timestamp.isoformat(), 'target_weights': {str(symbol): float(weight) for symbol, weight in targets.iloc[-1].to_dict().items()}, 'mid_prices': {str(symbol): float(price) for symbol, price in prices.iloc[-1].to_dict().items()}, 'source': compiled.metadata.get('source'), 'bundle_as_of': compiled.metadata.get('bundle_as_of'), 'compiled_metadata': compiled.metadata}
        finally:
    ...
    async def _net_deposit(self, address: str, *, fallback: float) -> float:
        ...
        if ledger_adapter is None:
            return fallback
        ok, net_deposit = await ledger_adapter.get_strategy_net_deposit(wallet_address=address)
        return _finite_float(net_deposit, fallback) if ok else fallback

    def _account_value(self, state: dict[str, Any]) -> float:
        cross_value = _finite_float((state.get('crossMarginSummary') or {}).get('accountValue'), 0.0)
        margin_value = _finite_float((state.get('marginSummary') or {}).get('accountValue'), 0.0)
    ...
    def _build_trade_plan(self, *, target_weights: dict[str, float], current_positions: dict[str, float], mids: dict[str, float], account_value: float, leverage: float, min_trade_usd: float, circuit_breaker: CircuitBreakerState | None=None) -> list[dict[str, Any]]:
        ...
        for symbol in symbols:
            ...
            if circuit_breaker is not None and account_value > 0.0:
                max_pos_frac = compute_position_size(risk_budget=circuit_breaker.max_risk_per_trade_pct, volatility=0.05, max_size=circuit_breaker.max_position_pct)
            ...
            if delta_usd < min_trade_usd:
                continue
            plan.append({'symbol': symbol, 'target_weight': round(_finite_float(target_weights.get(symbol), 0.0), 6), 'target_qty': round(target_qty, 8), 'current_qty': round(current_qty, 8), 'delta_qty': round(delta_qty, 8), 'delta_usd': round(delta_usd, 4), 'size': abs(delta_qty), 'is_buy': delta_qty > 0.0, 'reduce_only': False})
        return plan
