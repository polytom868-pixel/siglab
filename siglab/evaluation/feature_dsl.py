...
def load_feature_spec(
    ...
    *,
    ...
    if family is None:
        ...
        rsf: dict[str, list[str]] = {}
        for fn, fs in fams.items():
            aliases.update(dict(fs.get("aliases") or {}))
            rsf[fn] = list(fs.get("raw_series") or [])
        return {
            "aliases": aliases,
            "raw_series_by_family": rsf,
            "operators": list(_OPS),
        }
    fs_ = ts.get("families", {}).get(family, {})
    return {
        "aliases": dict(fs_.get("aliases") or {}),
        "raw_series": list(fs_.get("raw_series") or []),
        "operators": list(_OPS),
    }
...
def valid_expr(
    ...
    *,
    ...
    try:
        _eval(
            expression,
            raw_frames={n: pd.DataFrame() for n in raw_series},
            aliases=aliases,
            cache={},
            validate_only=True,
        )
...
def _eval(
    ...
    *,
    ...
    if expr in aliases:
        ...
        cache[expr] = result
        return result
    if expr in raw_frames:
        result = raw_frames[expr]
        cache[expr] = result
        return result
    if _ID_RE.match(expr):
        raise ValueError(f"Unknown feature token: {expr}")
    fn, args = _parse(expr)
    args_val = [
        _earg(
            t,
    ...
    result = _apply(fn, args_val, validate_only=validate_only)
    cache[expr] = result
    return result
...
def _parse(expr: str) -> tuple[str, list[str]]:
    if "(" not in expr or not expr.endswith(")"):
        raise ValueError(f"Invalid feature expression: {expr}")
...
def _op_mean_reversion_halflife(args, *, validate_only):
    ...
    delta = f.diff()
    beta = (
        delta.rolling(w).cov(lagged).div(lagged.rolling(w).var().replace(0.0, np.nan))
    )
    phi = 1.0 + beta
...
def _op_rsi(args, *, validate_only):
    ...
    alpha = 1.0 / max(w, 1)
    ag = gains.ewm(alpha=alpha, adjust=False, min_periods=w).mean()
    al = losses.ewm(alpha=alpha, adjust=False, min_periods=w).mean()
    rs = ag.div(al.replace(0.0, np.nan))
...
_OP_REG = dict[str, Any](
    [
        ("pct_change", _make_frame_periods_op("pct_change")),
        ("diff", _make_frame_periods_op("diff")),
        ("rolling_mean", _make_rolling_op("mean")),
        ("rolling_sum", _make_rolling_op("sum")),
        ("rolling_std", _make_rolling_op("std")),
        ("rolling_min", _make_rolling_op("min")),
        ("rolling_max", _make_rolling_op("max")),
        ("rolling_skew", _make_rolling_op("skew")),
        ("rolling_kurt", _make_rolling_op("kurt")),
...
def _bin(
    ...
    if isinstance(left, pd.DataFrame):
        return cast(pd.DataFrame, op(left, right))
    if isinstance(right, pd.DataFrame):
        if op is _sdiv:
            return cast(
                pd.DataFrame,
                _sdiv(
                    pd.DataFrame(float(left), index=right.index, columns=right.columns),
                    right,
                ),
            )
        return cast(
...
def _ap(
    ...
    if isinstance(left, pd.DataFrame):
        return (
            left,
            pd.DataFrame(cast(float, right), index=left.index, columns=left.columns),
        )
    if isinstance(right, pd.DataFrame):
...
def _afp(left: pd.DataFrame, right: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if lc == rc:
        return (li.reindex(columns=lc), ri.reindex(columns=rc))
    if len(lc) == 1 and lc[0] not in rc:
        return (_bcast(li, target_columns=rc), ri.reindex(columns=rc))
    if len(rc) == 1 and rc[0] not in lc:
        return (li.reindex(columns=lc), _bcast(ri, target_columns=lc))
    cc = [c for c in lc if c in rc]
    if cc:
        return (li.reindex(columns=cc), ri.reindex(columns=cc))
...
def _hurst(values: np.ndarray) -> float:
    ...
    return (
        float("nan")
        if len(tau) < 2
        else float(np.polyfit(np.log(lv), np.log(tau), 1)[0])
    )
...
def _kbf(
    ...
    *,
    ...
    for c in al.columns:
        y = al[c].astype(float).to_numpy()
        x = ar[c].astype(float).to_numpy()
        bs = np.full(len(y), np.nan, dtype=float)
        st = 0.0
        cov = 1.0
        for i, (yy, xx) in enumerate(zip(y, x)):
            if not np.isfinite(yy) or not np.isfinite(xx):
                continue
            if i == 0 and abs(xx) > 1e-12:
                st = float(yy) / float(xx)
            pc = cov + q
            iv = xx * xx * pc + r
            if not np.isfinite(iv) or iv <= 0.0:
                continue
            kg = pc * xx / iv
            st = float(st + kg * (yy - xx * st))
            cov = max(float((1.0 - kg * xx) * pc), 1e-10)
            bs[i] = st
        out[c] = bs
    return out
