...
from wayfinder_autolab.feature_dsl import load_feature_spec
...
def compute_spec_fingerprint(root_dir: Path) -> dict[str, Any]:
    tracked_files = [
        ...
        root_dir / "wayfinder_autolab" / "search" / "mutate.py",
        root_dir / "wayfinder_autolab" / "families.py",
    ]
...
def render_runbook() -> str:
    return "\n".join(
        [
            "# Runbook",
            "",
            "This workspace is the planner and writer contract for `directional_perps`.",
            "",
            "Always-loaded files:",
            "- `RUNBOOK.md`",
            "- `TASK.md`",
            ...
            "",
            "Rules:",
            "- Treat `cards/experiments/*.md` as canonical pre-audit candidate cards.",
            ...
            "- `iterations/<iteration>_<parent_hash>/candidate.json`",
...
def render_constraints(
    *,
    ...
    lines = [
        "# Constraints",
        "",
        "The validator clamps candidates after writing. Stay inside these ranges to avoid drift.",
        "",
        "Global directional-perps rules:",
        "- `lookback_days` is clamped to 365 and then fixed to 365 for evaluated candidates.",
