...
class OperatorPipeline:
    ...
    def evidence_to_decision(
        ...
        for record in evidence_records:
            ...
            # HOLD contributes weight but not directional score
        ...
        return TradeSignal(
            ...
            reasoning=(
                ...
                f"net={net:.4f}, max_conf={max_conf:.4f} \u2192 {consensus}"
            ),
        )
    ...
    def risk_check(
        ...
        for b in breach.breaches:
            reasons.append(
                f"Concentration breach: {b['strategy']} "
                f"alloc={b['allocation']:.1%} limit={b['limit']:.1%}"
            )
        ...
        if signal.size > max_size_usd:
            reasons.append(
                f"Signal size ${signal.size:.2f} exceeds risk-budgeted "
                f"cap ${max_size_usd:.2f}"
            )
    ...
    def position_to_paper(
        ...
        *,
        ...
        if self.paper_client is None:
            raise RuntimeError(
                "paper_client is required to place paper orders. "
                "Set paper_client in OperatorPipeline constructor "
