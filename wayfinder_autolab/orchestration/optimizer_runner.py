...
class OptunaOptimizerRunner:
    ...
    async def run(
        ...
        *,
        ...
        optuna_space = infer_optuna_space(candidate_payload)
        ...
        best_payload = dict(candidate_payload)
        best_params: dict[str, Any] = {}
        best_objective = -1e18
        ...
        for _ in range(int(getattr(self.settings, "optuna_trials", 20) or 20)):
            ...
            payload = clone_payload(candidate_payload)
            ...
            candidate = CandidateGraph.from_dict(validated_payload)
            try:
            ...
            row = {
                ...
                "promotion_score": objective_details.get("promotion_score"),
    ...
    async def _stability_sweep(
        ...
        *,
        ...
        if not bool(best_summary.get("passed")):
            return {"status": "skipped_failed_candidate", "neighbor_count": 0}
        parameters = list(optuna_space.get("parameters") or [])
        if not parameters:
            return {"status": "skipped_no_params", "neighbor_count": 0}

        low_payload = clone_payload(candidate_payload)
        high_payload = clone_payload(candidate_payload)
        low_changed = False
        high_changed = False
        for item in parameters:
            path = str(item.get("path") or "")
            current = get_path_value(candidate_payload, path)
            if current is None:
                continue
        ...
        for label, payload in neighbor_payloads:
            ...
            results.append(
                {
                    "label": label,
                    "candidate_hash": CandidateGraph.from_dict(validated_payload).strategy_hash(),
                    "objective": objective,
                    "passed": passed,
                    "status": status,
                    "summary": {
    ...
    def _warm_start_priority(
        ...
        *,
        ...
        payload = dict(row.get("candidate") or {})
        target_features = self._feature_signature(candidate_payload)
        payload_features = self._feature_signature(payload)
        same_feature_hash = int(payload_features == target_features)
        same_book = int(
            get_path_value(payload, "params.long_count") == get_path_value(candidate_payload, "params.long_count")
            and get_path_value(payload, "params.short_count") == get_path_value(candidate_payload, "params.short_count")
        )
...
def infer_optuna_space(candidate_payload: dict[str, Any]) -> dict[str, Any]:
    family = str(candidate_payload.get("family") or "")
    params: list[dict[str, Any]] = []
    risk = dict(candidate_payload.get("risk") or {})
    policy = dict(candidate_payload.get("params") or {})
    entry_gates = list(dict(candidate_payload.get("regime_gates") or {}).get("entry") or [])
    ...
    if family in {"perp_pair_trade_unlevered", "perp_pair_trade_levered"}:
        ...
        for path in ["params.entry_abs_score", "params.exit_abs_score", "params.flip_abs_score"]:
            add_float(path, get_path_value(candidate_payload, path), low=0.0, high=1.5)
