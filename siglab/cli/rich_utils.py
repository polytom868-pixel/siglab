...
def make_console(*, force_no_color: bool=False) -> Console:
    ...
    return Console(theme=SIGLAB_THEME, no_color=no_color, highlight=not no_color and is_tty, stderr=False, force_terminal=is_tty if not no_color else False, force_jupyter=False)
...
def make_table(title: str | None=None, *, show_lines: bool=False, header_style: str='bold', border_style: str='muted', row_styles: tuple[str, ...]=('', 'dim')) -> Table:
    """Create a consistently styled Rich Table."""
    return Table(title=title, show_lines=show_lines, header_style=header_style, border_style=border_style, row_styles=row_styles, expand=False)
