...
def add_subparser(
    ...
    start_parser.add_argument(
        "--session",
        default=None,
        help="Optional label for the session.",
    )
    start_parser.add_argument(
        "--sessions-dir",
        default=None,
        help="Directory for .npy session files.",
    )
    ...
    promote_parser.add_argument(
        ...
        type=float,
        default=None,
        help="Promotion score threshold.",
    )
    promote_parser.add_argument(
        "--consecutive-days",
        type=int,
        default=None,
        help="Required consecutive days above threshold.",
    )
    promote_parser.add_argument(
        "--min-trading-days",
        type=int,
        default=None,
        help="Minimum trading days required.",
    )
...
async def run_paper_status(args: argparse.Namespace) -> None:
    ...
    try:
        try:
            open_orders = (
                client.get_orders(args.session, status="OPEN")
...
async def run_paper_promote(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        result = {
            ...
            "trading_days": 0,
            "threshold": args.threshold or DEFAULT_PROMOTION_THRESHOLD,
            "consecutive_days_required": args.consecutive_days
            or DEFAULT_CONSECUTIVE_DAYS,
            "min_trading_days_required": args.min_trading_days
            or DEFAULT_MIN_TRADING_DAYS,
        }
        print_json(result)
        raise SystemExit(1)
