...
import wayfinder_autolab.search.select as select_mod
from wayfinder_autolab.models import CandidateGraph
from wayfinder_autolab.search.select import pick_deterministic_parent, rank_deterministic_candidates
...
def _row(candidate: CandidateGraph, *, quality: float, passed: bool = False) -> dict:
    return {
...
class DeterministicArchiveTests(unittest.TestCase):
    ...
    def test_rank_deterministic_candidates_keeps_anchor_and_adds_diversity(self) -> None:
        carry = _candidate(
            family="perp_multi_asset_carry",
            features=["funding_72h_mean", "funding_carry_to_vol"],
            symbols=["BTC", "ETH", "SOL", "HYPE"],
        ...
        ranked = rank_deterministic_candidates(
            candidates=[carry, carry_alt, basket, pair],
            parent=carry,
            recent_rows=recent_rows,
            seed_candidates=[carry, basket, pair],
            population_size=2,
        )
