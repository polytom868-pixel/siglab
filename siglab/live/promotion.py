...
def compute_sub_scores(metrics: dict[str, Any]) -> dict[str, float]:
    ...
    return {'pnl': _normalize_pnl(float(metrics.get('total_return', 0))), 'sharpe': _normalize_sharpe(float(metrics.get('sharpe', 0))), 'win_rate': _normalize_win_rate(float(metrics.get('win_rate', 0))), 'drawdown': _normalize_drawdown(float(metrics.get('max_drawdown', 0)))}

def compute_composite_score(metrics: dict[str, Any], weights: dict[str, float] | None=None) -> float:
...
def promotion_eligible(daily_metrics: list[dict[str, Any]], *, threshold: float | None=None, consecutive_days: int | None=None, min_trading_days: int | None=None, weights: dict[str, float] | None=None) -> tuple[bool, str]:
    ...
    if total_days < m:
        return (False, f'Minimum trading days not met: {total_days} < {m} (even perfect score needs {m} trading days)')
    ...
    if total_days >= c:
        ...
        if all(above):
            return (True, f'Composite score {latest:.4f} above threshold {t} for {c} consecutive days')
        below_count = sum((1 for s in recent_scores if s < t))
        return (False, f'Composite score below threshold {t} on {below_count} of last {c} days (latest: {latest:.4f})')
...
def extract_session_metrics(client: SoDEXPaperPerpsClient, session_id: str) -> dict[str, Any]:
    ...
    if not filled_orders:
        return {'total_return': 0.0, 'sharpe': 0.0, 'win_rate': 0.5, 'max_drawdown': 0.0, 'trade_count': 0, 'total_pnl': session.pnl}
    ...
    if len(equity) > 0:
        ...
        max_dd = 0.0
    return {'total_return': total_return, 'sharpe': sharpe, 'win_rate': win_rate, 'max_drawdown': max_dd, 'trade_count': total_trades, 'total_pnl': session.pnl}

def extract_daily_metrics(client: SoDEXPaperPerpsClient, session_id: str) -> list[dict[str, Any]]:
    ...
    for day in days_sorted:
        ...
        daily_list.append({'total_return': day_pnl, 'sharpe': 0.0, 'win_rate': win_rate_day, 'max_drawdown': min(0.0, day_pnl / cumulative_equity) if cumulative_equity > 0 else 0.0, 'day_pnl': day_pnl, 'trade_count': total_trades_day})
    return daily_list