...
def write_evidence_graph_html(
    ...
    html_parts: list[str] = [
        ...
        ".node{display:inline-block;padding:4px 10px;margin:4px;border-radius:4px}"
        ".source{background:#ddf}"
        ".symbol{background:#dfd}"
        "</style></head><body>",
    ...
    for n in nodes:
        html_parts.append(
            f'<span class="node {n["kind"]}">{n["label"]}</span>'
        )
    html_parts.append("</div><h2>Edges</h2><ul>")
    for e in edges:
        html_parts.append(
            f"<li>{e['source']} → {e['target']} ({e['direction']}, "
            f"confidence={e['confidence']})</li>"
        )
