...
from wayfinder_autolab.search.select import _row_quality
...
""".strip()
            ...
            (artifact_dir / "directional_perps" / "winner.json").write_text(
                json.dumps(
                    {
            ...
            (artifact_dir / "directional_perps" / "loser.json").write_text(
                json.dumps(
                    {
    ...
    def test_propose_does_not_mix_deterministic_variants_into_llm_batches(self) -> None:
        mutator = object.__new__(CandidateMutator)
        mutator.settings = SimpleNamespace(
            root_dir=REPO_ROOT
        )
        mutator.kimi = SimpleNamespace(is_configured=True)
        ...
        llm_candidate = CandidateGraph.from_dict(
            {
                "track": "directional_perps",
        ...
        deterministic_candidate = CandidateGraph.from_dict(
            {
                "track": "directional_perps",
                "family": "perp_pair_trade_unlevered",
        ...
        mutator._llm_proposals = lambda **_kwargs: asyncio.sleep(0, result=[llm_candidate])
        mutator._deterministic_variants = lambda *_args, **_kwargs: [deterministic_candidate]
        mutator._validate_candidate = lambda **kwargs: kwargs["candidate"]

        proposals = asyncio.run(
            CandidateMutator.propose(
                mutator,
                track="directional_perps",
                parent=parent,
                research_summary={},
    ...
    def test_compact_memory_packet_keeps_validation_leaders(self) -> None:
        mutator = object.__new__(CandidateMutator)
        mutator.settings = SimpleNamespace()
        mutator.kimi = SimpleNamespace()
        mutator.last_llm_trace = None

        compact = CandidateMutator._compact_memory_packet(
            mutator,
            {
        ...
        self.assertEqual(compact["validation_leaders"][0]["candidate_hash"], "abc123")
        self.assertEqual(compact["validation_leaders"][0]["trade_style"], "continuation")
        self.assertEqual(compact["outstanding_runs"][0]["candidate_hash"], "run123")
    ...
    def test_compact_research_summary_keeps_pair_calibration(self) -> None:
        mutator = object.__new__(CandidateMutator)
        compact = CandidateMutator._compact_research_summary(
            mutator,
            {
                "track": "directional_perps",
                "pair_calibration": {
    ...
    def test_pair_formula_examples_use_symmetric_asset_inputs(self) -> None:
        mutator = object.__new__(CandidateMutator)
        examples = CandidateMutator._formula_examples(
            mutator,
            "directional_perps",
    ...
    def test_pair_feature_idea_examples_include_interpretable_patterns(self) -> None:
        mutator = object.__new__(CandidateMutator)
        ideas = CandidateMutator._feature_idea_examples(
            mutator,
            "directional_perps",
            "perp_pair_trade_unlevered",
        )
    ...
    def test_basket_and_directional_examples_include_carry_overlays(self) -> None:
        mutator = object.__new__(CandidateMutator)
        basket_examples = CandidateMutator._formula_examples(
            mutator,
            "directional_perps",
            "perp_basket_neutral_unlevered",
        )
        directional_ideas = CandidateMutator._feature_idea_examples(
            mutator,
            "directional_perps",
    ...
    def test_write_llm_exchange_log_persists_payload_and_output(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mutator.kimi = SimpleNamespace(
                ...
                last_exchange={"final_content": "{\"candidates\":[]}"},
            )
            parent = CandidateGraph.from_dict(
                {
                    "track": "directional_perps",
            ...
            path = CandidateMutator._write_llm_exchange_log(
                mutator,
                track="directional_perps",
                parent=parent,
                system_prompt="sys",
                user_payload={"task": "Return exactly 1 candidate"},
    ...
    def test_validate_candidate_normalizes_pair_regime_gates(self) -> None:
        mutator = object.__new__(CandidateMutator)
        mutator.settings = SimpleNamespace(
            root_dir=REPO_ROOT
        )
        mutator.kimi = SimpleNamespace(is_configured=False)
        mutator.last_llm_trace = None
        mutator.last_llm_log_path = None

        track = "directional_perps"
        allowed_families = CandidateMutator._allowed_families(mutator, track, family=None)
        allowed_features_by_family = CandidateMutator._allowed_features_by_family(
            mutator, track, family=None
        )
        family_defaults = CandidateMutator._family_defaults(mutator, track, family=None)
        ...
        fixed = CandidateMutator._validate_candidate(
            mutator,
            candidate=candidate,
            track=track,
    ...
    def test_validate_candidate_keeps_scientific_notation_regime_gate(self) -> None:
        mutator = object.__new__(CandidateMutator)
        mutator.settings = SimpleNamespace(
            root_dir=REPO_ROOT
        )
    ...
    def test_validate_candidate_keeps_regime_gates_for_basket_neutral_family(self) -> None:
        mutator = object.__new__(CandidateMutator)
        mutator.settings = SimpleNamespace(
            root_dir=REPO_ROOT
        )
    ...
    def test_validate_candidate_does_not_truncate_five_pair_regime_gates(self) -> None:
        mutator = object.__new__(CandidateMutator)
        mutator.settings = SimpleNamespace(
            root_dir=REPO_ROOT
        )
        ...
        fixed = CandidateMutator._validate_candidate(
            ...
            candidate=candidate,
            track="directional_perps",
            allowed_families=CandidateMutator._allowed_families(mutator, "directional_perps", family=None),
            allowed_features_by_family=CandidateMutator._allowed_features_by_family(
                mutator,
                "directional_perps",
                family=None,
            ),
            family_defaults=CandidateMutator._family_defaults(
                mutator,
                "directional_perps",
                family=None,
            ),
    ...
    def test_validate_candidate_normalizes_multi_asset_carry_family(self) -> None:
        mutator = object.__new__(CandidateMutator)
        mutator.settings = SimpleNamespace(
            root_dir=REPO_ROOT
        )
    ...
    def test_validate_candidate_rejects_single_asset_alias_for_pair_family(self) -> None:
        mutator = object.__new__(CandidateMutator)
        mutator.settings = SimpleNamespace(
            root_dir=REPO_ROOT
        )
        ...
        validated = CandidateMutator._validate_candidate(
            mutator,
            candidate=candidate,
            track="directional_perps",
    ...
    def test_validate_candidate_keeps_pair_safe_one_sided_carry_alias(self) -> None:
        mutator = object.__new__(CandidateMutator)
        mutator.settings = SimpleNamespace(
            root_dir=REPO_ROOT
        )
    ...
    def test_validate_candidate_normalizes_trade_style_for_pair_family(self) -> None:
        mutator = object.__new__(CandidateMutator)
        mutator.settings = SimpleNamespace(
            root_dir=REPO_ROOT
        )
    ...
    def test_extract_llm_candidate_rows_accepts_top_level_single_candidate(self) -> None:
        mutator = object.__new__(CandidateMutator)

        rows = CandidateMutator._extract_llm_candidate_rows(
            mutator,
            {
                "track": "directional_perps",
                "family": "perp_pair_trade_unlevered",
                "hypothesis": "top level candidate",
                "neutrality_basis": "none",
                "features": ["pair_corr_72h"],
                "universe": {"basis_groups": ["ETH", "BTC"], "max_symbols": 2},
                "risk": {},
        ...
        self.assertEqual(rows[0]["hypothesis"], "top level candidate")

    def test_extract_llm_candidate_rows_accepts_nested_single_candidate(self) -> None:
        mutator = object.__new__(CandidateMutator)
        ...
        self.assertEqual(len(rows), 1)
        self.assertEqual(rows[0]["hypothesis"], "nested candidate")
    ...
    def test_pair_deterministic_variants_include_cross_family_and_novelty_seeds(self) -> None:
        ...
        variants = CandidateMutator._deterministic_variants(
            mutator,
            "directional_perps",
            parent,
        ...
        self.assertTrue(any(candidate.family == "perp_multi_asset_decision" for candidate in variants[:4]))
        self.assertTrue(
            any(candidate.universe.basis_groups != ["ETH", "BTC"] for candidate in variants[:6] if candidate.family == "perp_pair_trade_unlevered")
        )
        self.assertTrue(
            any("neg(pair_kalman_residual_z_72h)" in candidate.features for candidate in variants)
        )
    ...
    def test_cross_sectional_deterministic_variants_rotate_alternate_baskets(self) -> None:
        ...
        seen_baskets = {
            tuple(candidate.universe.basis_groups)
            for candidate in variants[:6]
            if candidate.family == "perp_multi_asset_carry"
        }
