...
const DISPLAY_CAPITAL_STORAGE_KEY = "autolab.displayCapitalUsd";
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const candidateHash = getCandidateHash();
  if (!candidateHash) {
    renderMissing("No experiment hash was found in the URL.");
    return;
  }

  const response = await fetch(`/api/experiments/${encodeURIComponent(candidateHash)}/series`, {
    cache: "no-store",
  });
  if (!response.ok) {
...
function renderPage() {
  ...
  document.title = `${experiment.family || "Experiment"} ${experiment.candidate_hash || ""}`.trim();
  document.getElementById("experimentTitle").textContent =
    `${experiment.family || "Experiment"} ${experiment.candidate_hash || ""}`.trim();
...
function renderPromotion(experiment) {
  const panel = document.getElementById("promotionPanel");
  const readiness = experiment.promotion_readiness || { supported: false, reasons: [], warnings: [] };
  const promotion = experiment.live_promotion || null;
  const reasons = readiness.reasons || [];
  const warnings = readiness.warnings || [];
  const promotedMarkup = promotion
    ? `
      <div class="detail-block">
        <h3>Latest Promotion</h3>
        <div class="kv">
          <div class="key">Strategy</div><div>${escapeHtml(promotion.strategy_name || "n/a")}</div>
          <div class="key">Scheduled</div><div>${promotion.scheduled ? "yes" : "no"}</div>
          <div class="key">Dry Run</div><div>${promotion.dry_run ? "yes" : "no"}</div>
          <div class="key">Job</div><div>${escapeHtml(promotion.job_name || "n/a")}</div>
          <div class="key">Wallet</div><div>${escapeHtml(promotion.wallet_label || "n/a")}</div>
          <div class="key">Spec</div><div class="mono">${escapeHtml(promotion.spec_path || "n/a")}</div>
        </div>
      </div>
    `
    : "";
  ...
  const unsupportedMarkup = !readiness.supported
    ...
    : `
      <form id="promotionForm" class="promotion-form">
        <div class="filters promotion-grid">
          <label>
            Wallet Label
            <input id="promotionWalletLabel" type="text" placeholder="basis_trading_strategy" />
          </label>
          <label>
            Job Name
            <input id="promotionJobName" type="text" placeholder="autolab-job-name" />
          </label>
          <label>
            Interval Seconds
            <input id="promotionInterval" type="number" min="60" step="60" value="600" />
          </label>
          <label>
            Config Path
            <input id="promotionConfigPath" type="text" placeholder="use default config" />
          </label>
          <label class="checkbox">
            <input id="promotionSchedule" type="checkbox" />
            Schedule runner job
          </label>
          <label class="checkbox">
            <input id="promotionLive" type="checkbox" />
            Live trading
          </label>
          <label class="checkbox">
            <input id="promotionLlmFinalize" type="checkbox" />
            Kimi finalize notes
          </label>
          <button type="submit">Promote</button>
        </div>
      </form>
      <div id="promotionResult" class="detail-copy"></div>
    `;
  ...
  if (form) {
    form.addEventListener("submit", async (event) => {
      ...
      await submitPromotion(experiment.candidate_hash);
    });
  }
}

async function submitPromotion(candidateHash) {
  const resultNode = document.getElementById("promotionResult");
  if (resultNode) {
    resultNode.textContent = "Promoting...";
  }
  const payload = {
    wallet_label: document.getElementById("promotionWalletLabel")?.value || null,
    job_name: document.getElementById("promotionJobName")?.value || null,
    interval_seconds: parseOptionalNumber(document.getElementById("promotionInterval")?.value),
    config_path: document.getElementById("promotionConfigPath")?.value || null,
    schedule: Boolean(document.getElementById("promotionSchedule")?.checked),
    llm_finalize: Boolean(document.getElementById("promotionLlmFinalize")?.checked),
    dry_run: !Boolean(document.getElementById("promotionLive")?.checked),
  };
  try {
    const response = await fetch(`/api/experiments/${encodeURIComponent(candidateHash)}/promote`, {
      method: "POST",
      cache: "no-store",
      headers: {
    ...
    if (resultNode) {
      resultNode.textContent = `Promoted as ${data.promotion?.strategy_name || "generated strategy"}.`;
    }
    const refreshed = await fetch(`/api/experiments/${encodeURIComponent(candidateHash)}/series`, {
      cache: "no-store",
    });
    if (refreshed.ok) {
    ...
    if (resultNode) {
      resultNode.textContent = `Promotion failed: ${error.message || error}`;
    }
  }
}
  ...
  const cards = [
    {
      ...
      detail: "Primary selection metric used for promotion.",
    },
    {
      label: "Median Sharpe",
