...
class SoDEXSignedClientTests(unittest.IsolatedAsyncioTestCase):
    ...
    def _make_client(*, dry_run: bool = True, client: object | None = None) -> SoDEXSignedPerpsClient:
        return SoDEXSignedPerpsClient(
            ...
            account_id=1001,
            signer=_Signer(),
            nonce_manager=SoDEXNonceManager(environment="testnet"),
            environment="testnet",
            dry_run=dry_run,
            client=client,
        )
    ...
    async def test_send_signed_request_skips_in_dry_run(self) -> None:
        client = self._make_client()

        result = await client.send_signed_request(
            SoDEXSignedRequest(
                method="POST",
                path="/trade/orders",
                body=perps_update_leverage_body(account_id=1001, symbol_id=1, leverage=5, margin_mode=1),
        ...
        self.assertIn("/trade/orders", result["url"])
