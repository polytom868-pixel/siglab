...
class NextBarBiasTests(unittest.IsolatedAsyncioTestCase):
    def _settings(self) -> SiglabConfig:
        return SiglabConfig(
            root_dir=Path("/tmp"),
    ...
    async def test_evaluator_shifts_positions_and_drops_last_bar(self) -> None:
        ...
        with patch(
            "siglab.evaluator.core.compile_spec",
            new=AsyncMock(return_value=compiled),
        ), patch(
            "siglab.evaluator.core.run_backtest",
            side_effect=fake_run_backtest,
        ):
            evaluation = await evaluator.evaluate(spec)
    ...
    async def test_pair_policy_sweep_freezes_params_from_train_windows_only(self) -> None:
        ...
        self.assertAlmostEqual(evaluation["spec"]["params"]["entry_abs_score"], 0.1)
        self.assertAlmostEqual(evaluation["spec"]["params"]["min_abs_score"], 0.1)
    ...
    async def test_fast_mode_caps_pair_policy_sweep_trials_and_windows(self) -> None:
        ...
        with patch(
            ...
            evaluation = await evaluator.evaluate(spec, fast_mode=True)
