...
def _narrative_header(run_meta: dict) -> str:
    """Return a short header identifying the evaluation run."""
...
def _narrative_performance_summary(stats: dict) -> str:
    ...
    lines = [
        "=== Performance Summary ===",
        f"Total Return: {_fmt(total_ret)}" if total_ret is not None else "Total Return: N/A",
...
def _narrative_drawdown_analysis(drawdown: dict) -> str:
    ...
    lines = [
        "=== Drawdown Analysis ===",
        f"Max drawdown: {_fmt(max_dd)} over {int(bars)} bar(s)" if max_dd is not None else "Max drawdown: N/A",
    ...
    if pre_peak.get("trade_count", 0) > 0:
        lines.append(
            f"Pre-drawdown: {pre_peak['trade_count']} trades, "
            f"win rate {_fmt(pre_peak.get('win_rate', 0.0) * 100.0)}%, "
            f"avg return {_fmt(pre_peak.get('avg_return', 0.0))}"
        )
...
def build_signal_narrative(canonical_run: dict) -> str:
    ...
    # Exemplar trades from pre_audit_context_pack
