...
from siglab.workspace.indexes import append_jsonl, ensure_index, load_jsonl, maybe_compact
...
class WorkspaceBuilder:
    ...
    def _session_scope_kwargs(self, session: WorkspaceSession) -> dict[str, str]:
        if session.memory_scope == "track_shared":
            return {}
    ...
    def update_iteration(
        ...
        *,
        ...
        rows = self.ancestry.dashboard_rows(
            track=session.track,
        ...
        return {
            ...
            "base_spec_path": iteration_dir / "base_spec.yaml",
            "spec_patch_path": iteration_dir / "spec_patch.json",
            "spec_after_patch_path": iteration_dir / "spec_after_patch.yaml",
            "spec_json_path": iteration_dir / "spec.json",
    ...
    def record_experiment(self, *, session: WorkspaceSession, spec_hash: str, iteration_number: int) -> str | None:
        row = self.ancestry.experiment_detail(spec_hash)
        if row is None:
            return None
        artifact = dict(row.get("artifact") or {})
        canonical_path = session.cards_dir / "experiments" / f"{spec_hash}.md"
        ...
        proxy_path = session.cards_dir / proxy_kind / f"{spec_hash}.md"
        ...
        search_text = " ".join(
            [
                ...
                str(dict(row.get("spec") or {}).get("hypothesis") or ""),
                " ".join(str(feature) for feature in dict(row.get("spec") or {}).get("features") or []),
    ...
    def record_lesson_card(
        ...
        *,
        ...
        path = session.cards_dir / "reflections" / f"{spec_hash}.md"
    ...
    def refresh_frontier_files(self, session: WorkspaceSession) -> None:
        ...
        self._write_if_changed(
            session.current_dir / "incumbent_spec.yaml",
            self._render_incumbent_spec(session=session),
        )
        self._write_if_changed(
    ...
    def _materialize_existing_cards(self, session: WorkspaceSession) -> None:
        ...
        for index, row in enumerate(rows, start=1):
            ...
            if not spec_hash:
                continue
            self.record_experiment(session=session, spec_hash=spec_hash, iteration_number=index)
    ...
    def _frontier_digest(self, *, session: WorkspaceSession, rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for row in rows:
            ...
            for feature in dict(row.get("spec") or {}).get("features") or []:
    ...
    def _render_frontier_brief(self, frontier_digest: dict[str, Any]) -> str:
        ...
        for anchor in anchors:
            lines.append(
                f"- `{anchor.get('spec_hash')}` `{anchor.get('family')}` pre_audit={anchor.get('pre_audit_canonical_total_return')} validation={anchor.get('validation_total_return')}"
            )
        if not anchors:
            lines.append("- none")
    ...
    def _render_parent_card(self, *, session: WorkspaceSession, parent: SignalSpec) -> str:
        detail = self.ancestry.experiment_detail(parent.strategy_hash())
        ...
        return "\n".join(
            [
                "# Parent Card",
                "",
                "Parent is a seed or has not been evaluated in this ancestry store yet.",
                f"Family: `{parent.family}`",
                f"Hypothesis: {parent.hypothesis or 'n/a'}",
                "```json",
                json.dumps(spec, indent=2, ensure_ascii=True, default=str),
                "```",
            ]
        ).strip() + "\n"
    ...
    def _render_incumbent_spec(self, *, session: WorkspaceSession) -> str:
        best = self.ancestry.best(
            session.track,
            run_session_id=session.run_session_id if session.memory_scope != "track_shared" else None,
        )
        if best is not None:
            payload = dict(best.get("spec") or {})
        else:
            payload = self._seed_spec_payload(track=session.track, family=None)
    ...
    def _family_incumbents_payload(
        ...
        *,
        ...
        for family in session.families:
            best_row = self._best_ancestry_row(session=session, family=family)
            if best_row is not None:
                payload[family] = {
                    "source": "ancestry",
                    "spec_hash": best_row.get("spec_hash"),
                    "aggregate_score": dict(best_row.get("summary") or {}).get("aggregate_score"),
                    "passed": bool(best_row.get("passed")),
                    "deployd": bool(best_row.get("deployd")),
                    "spec": dict(best_row.get("spec") or {}),
                    "experiment_ref": f"cards/experiments/{best_row.get('spec_hash')}.md",
                }
                continue
            seed_payload = self._seed_spec_payload(track=session.track, family=family)
            payload[family] = {
                "source": "seed",
                "spec_hash": None,
    ...
    def _trial_entry_from_row(
        ...
        *,
        ...
        reflection_ref = f"cards/reflections/{spec_hash}.md"
        base_ref = self._relative_workspace_ref(
            session=session,
            value=trial.get("base_spec_path") or workspace.get("base_spec_path"),
        )
        patch_ref = self._relative_workspace_ref(
            session=session,
            value=trial.get("spec_patch_path") or workspace.get("spec_patch_path"),
        )
        spec_after_patch_ref = self._relative_workspace_ref(
            session=session,
            value=trial.get("spec_after_patch_path") or workspace.get("spec_after_patch_path"),
        )
        ...
        return {
            ...
            "reflection_excerpt": self._reflection_excerpt(session=session, spec_hash=spec_hash),
            ...
            "deployment_score": trial.get("deployment_score", generalization.get("deployment_score")),
            "audit_alignment": str(
            ...
            "refs": {
                ...
                "base_spec": base_ref,
                "spec_patch": patch_ref,
                "spec_after_patch": spec_after_patch_ref,
    ...
    def _render_recent_trials(self, recent_trials: list[dict[str, Any]]) -> str:
        ...
        if not recent_trials:
            lines.extend(
                [
                    ...
                    "",
                    "Use the incumbent spec plus manifests to propose the first structural test.",
                ]
            )
            return "\n".join(lines).strip() + "\n"
        ...
        for entry in recent_trials:
            ...
            lines.extend(
                [
                    f"## `{entry.get('spec_hash')}` `{entry.get('family')}` `{entry.get('outcome')}`",
            ...
            if patch_summary:
                lines.append(f"- claude_patch: {'; '.join(patch_summary[:6])}")
            ...
            if entry.get("deployment_score") is not None:
                generalization_bits.append(
                    f"deployment_score=`{self._format_metric(entry.get('deployment_score'))}`"
                )
        ...
        *,
        ...
        for row in rows:
            ...
            index_rows.append(
                {
                    "kind": "trial",
                    "path": refs.get("optuna_best") or refs.get("spec_after_patch") or refs.get("structure_spec") or "",
                    ...
                    "search_text": " ".join(
                        [
                            ...
                            str(entry.get("deployment_score") or ""),
    ...
    def _seed_spec_payload(self, *, track: str, family: str | None) -> dict[str, Any]:
        try:
            specs = self.mutator.load_seed_specs(track, family=family)
        except Exception:  # noqa: BLE001
            return {}
        if not specs:
            return {}
        return specs[0].canonical_dict()
    ...
    def _reflection_excerpt(self, *, session: WorkspaceSession, spec_hash: str) -> str:
        path = session.cards_dir / "reflections" / f"{spec_hash}.md"
        if not path.exists():
            return ""
        try:
    ...
    def _build_motif_registry(self, *, rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for row in recent_rows:
            ...
            signature = motif_signature(spec)
            ...
            spec_hash = str(row.get("spec_hash") or "")
            if spec_hash:
                entry["recent_specs"].append(spec_hash)
        motif_rows = sorted(
            motifs.values(),
            key=lambda item: (
        ...
        for item in motif_rows:
            item["recent_specs"] = list(item.get("recent_specs") or [])[-5:]
    ...
    def _is_regime_focused_carry_attempt(self, row: dict[str, Any]) -> bool:
        spec = dict(row.get("spec") or {})
        features = [str(feature) for feature in list(spec.get("features") or [])]
