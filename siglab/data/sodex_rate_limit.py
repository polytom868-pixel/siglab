...
class SoDEXWeightScheduler:
    ...
    def __init__(self, *, budget: int=SODEX_WEIGHT_BUDGET_PER_MINUTE, window_seconds: float=60.0, now: Callable[[], float] | None=None, sleep: Callable[[float], Awaitable[None] | None] | None=None) -> None:
    ...
    def snapshot(self) -> dict[str, float | int]:
        self._prune()
        return {'budget': self.budget, 'window_seconds': self.window_seconds, 'used_weight': self._used_weight(), 'available_weight': max(0, self.budget - self._used_weight()), 'admitted': self.metrics.admitted, 'rejected': self.metrics.rejected, 'slept': self.metrics.slept, 'total_weight': self.metrics.total_weight}
