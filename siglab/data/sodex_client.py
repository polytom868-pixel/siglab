...
class SoDEXPublicPerpsClient:
    def __init__(self, *, base_url: str='https://mainnet-gw.sodex.dev/api/v1/perps', timeout_s: float=10.0, retries: int=3, client: httpx.AsyncClient | None=None, weight_scheduler: SoDEXWeightScheduler | None=None) -> None:
    ...
    async def _list(self, path: str, endpoint: str, *, param_key: str | None=None, param_value: str | None=None) -> list[dict[str, Any]]:
        params = {param_key: param_value} if param_key and param_value else None
        return self._rows(await self._request('GET', path, endpoint=endpoint, params=params, weight=SoDEXWeightScheduler.documented_weight(endpoint)), endpoint)
    ...
    async def orderbook(self, *, symbol: str, limit: int | None=None) -> dict[str, Any]:
        ...
        payload = await self._request('GET', f'/markets/{symbol}/orderbook', endpoint='perps.orderbook', params=params, weight=SoDEXWeightScheduler.documented_weight('perps.orderbook'))
        ...
        return result
    async def klines(self, *, symbol: str, interval: str, start_time: int | None=None, end_time: int | None=None, limit: int | None=None) -> list[dict[str, Any]]:
        ...
        return self._rows(await self._request('GET', f'/markets/{symbol}/klines', endpoint='perps.klines', params=params, weight=SoDEXWeightScheduler.documented_weight('perps.klines')), 'perps.klines')
    async def trades(self, *, symbol: str, limit: int | None=None) -> list[dict[str, Any]]:
        params = {'limit': int(limit)} if limit is not None else None
        return self._rows(await self._request('GET', f'/markets/{symbol}/trades', endpoint='perps.trades', params=params, weight=SoDEXWeightScheduler.documented_weight('perps.trades')), 'perps.trades')
    async def funding_history(self, symbol: str, start_time: int, end_time: int) -> list[dict[str, Any]]:
        """Fetch historical funding rates for *symbol* between *start_time* and *end_time* (ms epochs)."""
        return self._rows(await self._request('GET', f'/markets/{symbol}/fundingRate', endpoint='perps.funding_history', params={'startTime': int(start_time), 'endTime': int(end_time)}, weight=SoDEXWeightScheduler.documented_weight('perps.funding_history')), 'perps.funding_history')
    async def account_balances(self, *, user_address: str, account_id: int | None=None) -> dict[str, Any]:
        return await self._account_object(endpoint='perps.account_balances', path=f'/accounts/{_validate_evm_address(user_address)}/balances', account_id=account_id)
    async def account_orders(self, *, user_address: str, symbol: str | None=None, account_id: int | None=None) -> dict[str, Any]:
        params: dict[str, Any] = {}
        if symbol:
            params['symbol'] = symbol
        if account_id is not None:
            params['accountID'] = int(account_id)
        payload = await self._request('GET', f'/accounts/{_validate_evm_address(user_address)}/orders', endpoint='perps.account_orders', params=params or None, weight=SoDEXWeightScheduler.documented_weight('perps.account_orders'))
        data = payload.get('data')
    ...
    async def account_positions(self, *, user_address: str, account_id: int | None=None) -> dict[str, Any]:
        return await self._account_object(endpoint='perps.account_positions', path=f'/accounts/{_validate_evm_address(user_address)}/positions', account_id=account_id)
    async def account_state(self, *, user_address: str, account_id: int | None=None) -> dict[str, Any]:
        return await self._account_object(endpoint='perps.account_state', path=f'/accounts/{_validate_evm_address(user_address)}/state', account_id=account_id)
    async def _account_object(self, *, endpoint: str, path: str, account_id: int | None=None) -> dict[str, Any]:
        ...
        payload = await self._request('GET', path, endpoint=endpoint, params=params, weight=SoDEXWeightScheduler.documented_weight(endpoint))
        data = payload.get('data')
        if not isinstance(data, dict):
            raise SoDEXFormatError(f'{endpoint} data was not an object', payload=payload)
        return dict(data)
    async def _request(self, method: str, path: str, *, endpoint: str, params: dict[str, Any] | None=None, weight: int=20) -> dict[str, Any]:
        ...
        for attempt in range(self.retries + 1):
            ...
            try:
                response = await self._http().request(method, f'{self.base_url}/{path.lstrip('/')}', params=params, headers={'Accept': 'application/json'}, timeout=self.timeout_s)
    ...
    def metrics_snapshot(self) -> dict[str, Any]:
        ...
        for name, metrics in self._metrics.items():
            ...
            endpoints[name] = {'p50_ms': _percentile(latencies, 50), 'p95_ms': _percentile(latencies, 95), 'attempts': metrics.attempts, 'success_rate': metrics.successes / attempts, 'retry_count': metrics.retries, '429_count': metrics.rate_limits, 'transport_failures': metrics.transport_failures}
    ...
    def _checked_payload(self, response: httpx.Response, endpoint: str) -> dict[str, Any]:
        ...
        if code not in (0, '0'):
            raise SoDEXUpstreamError(f'{endpoint} business error: {payload.get('error') or payload.get('message') or code}', status_code=status, payload=payload)
        return payload
