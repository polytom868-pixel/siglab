...
FUNCTION_OPERATORS = ('pct_change', 'diff', 'ema', 'rolling_mean', 'rolling_sum', 'rolling_std', 'rolling_zscore', 'rolling_min', 'rolling_max', 'rolling_skew', 'rolling_kurt', 'rolling_corr', 'rolling_autocorr', 'rolling_beta', 'rolling_hurst', 'mean_reversion_halflife', 'kalman_beta', 'kalman_residual', 'rsi', 'sub', 'mul', 'div', 'abs', 'log', 'clip', 'sign_flip_prob')
...
def load_feature_spec(root_dir: Path, *, track: str, family: str | None=None) -> dict[str, Any]:
    ...
    if family is None:
        ...
        return {'aliases': aliases, 'raw_series_by_family': raw_series_by_family, 'operators': list(FUNCTION_OPERATORS)}
    family_spec = track_spec.get('families', {}).get(family, {})
    return {'aliases': dict(family_spec.get('aliases') or {}), 'raw_series': list(family_spec.get('raw_series') or []), 'operators': list(FUNCTION_OPERATORS)}

def is_valid_feature_expression(expression: str, *, aliases: dict[str, str], raw_series: set[str]) -> bool:
    try:
        _evaluate_feature(expression, raw_frames={name: pd.DataFrame() for name in raw_series}, aliases=aliases, cache={}, validate_only=True)
...
def resolve_feature_frames(features: list[str], *, aliases: dict[str, str], raw_frames: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
    ...
    for feature in features:
        resolved[feature] = _evaluate_feature(feature, raw_frames=raw_frames, aliases=aliases, cache=cache, validate_only=False)
    return resolved

def _evaluate_feature(expression: str, *, raw_frames: dict[str, pd.DataFrame], aliases: dict[str, str], cache: dict[str, pd.DataFrame], validate_only: bool) -> pd.DataFrame:
    expression = expression.strip()
    if expression in cache:
        return cache[expression]
    if expression in aliases:
        result = _evaluate_feature(aliases[expression], raw_frames=raw_frames, aliases=aliases, cache=cache, validate_only=validate_only)
    ...
    args = [_evaluate_arg(token, raw_frames=raw_frames, aliases=aliases, cache=cache, validate_only=validate_only) for token in arg_tokens]
    result = _apply_operator(function_name, args, validate_only=validate_only)
    cache[expression] = result
    return result

def _evaluate_arg(token: str, *, raw_frames: dict[str, pd.DataFrame], aliases: dict[str, str], cache: dict[str, pd.DataFrame], validate_only: bool) -> pd.DataFrame | float:
    stripped = token.strip()
    if _NUMBER_RE.match(stripped):
        return float(stripped)
    return _evaluate_feature(stripped, raw_frames=raw_frames, aliases=aliases, cache=cache, validate_only=validate_only)
...
_OPERATOR_REGISTRY: dict[str, Any] = {'pct_change': _op_pct_change, 'diff': _op_diff, 'ema': _op_ema, 'rolling_mean': _op_rolling_mean, 'rolling_sum': _op_rolling_sum, 'rolling_std': _op_rolling_std, 'rolling_zscore': _op_rolling_zscore, 'rolling_min': _op_rolling_min, 'rolling_max': _op_rolling_max, 'rolling_skew': _op_rolling_skew, 'rolling_kurt': _op_rolling_kurt, 'rolling_corr': _op_rolling_corr, 'rolling_autocorr': _op_rolling_autocorr, 'rolling_beta': _op_rolling_beta, 'rolling_hurst': _op_rolling_hurst, 'mean_reversion_halflife': _op_mean_reversion_halflife, 'kalman_beta': _op_kalman_beta, 'kalman_residual': _op_kalman_residual, 'rsi': _op_rsi, 'sign_flip_prob': _op_sign_flip_prob, 'abs': _op_abs, 'log': _op_log, 'clip': _op_clip, 'sub': _op_sub, 'mul': _op_mul, 'div': _op_div}

def _apply_operator(function_name: str, args: list[pd.DataFrame | float], *, validate_only: bool) -> pd.DataFrame:
...
def _expect_two_frames_and_int(args: list[pd.DataFrame | float]) -> tuple[pd.DataFrame, pd.DataFrame, int]:
    if len(args) != 3 or not isinstance(args[0], pd.DataFrame) or (not isinstance(args[1], pd.DataFrame)) or (not isinstance(args[2], float)):
        raise ValueError('Expected frame, frame, and numeric window')
    return (args[0], args[1], int(args[2]))

def _expect_frame_and_two_ints(args: list[pd.DataFrame | float]) -> tuple[pd.DataFrame, int, int]:
    if len(args) != 3 or not isinstance(args[0], pd.DataFrame) or (not isinstance(args[1], float)) or (not isinstance(args[2], float)):
        raise ValueError('Expected frame and two numeric arguments')
    return (args[0], int(args[1]), int(args[2]))

def _expect_kalman_args(args: list[pd.DataFrame | float]) -> tuple[pd.DataFrame, pd.DataFrame, float, float]:
    if len(args) == 2 and isinstance(args[0], pd.DataFrame) and isinstance(args[1], pd.DataFrame):
        return (args[0], args[1], 1e-05, 0.001)
    if len(args) == 4 and isinstance(args[0], pd.DataFrame) and isinstance(args[1], pd.DataFrame) and isinstance(args[2], float) and isinstance(args[3], float):
...
def _binary(left: pd.DataFrame | float, right: pd.DataFrame | float, operation: Callable[[Any, Any], pd.DataFrame]) -> pd.DataFrame:
...
def _aligned_pair(left: pd.DataFrame | float, right: pd.DataFrame | float) -> tuple[pd.DataFrame, pd.DataFrame]:
    if isinstance(left, pd.DataFrame) and isinstance(right, pd.DataFrame):
...
def _aligned_frame_pair(left: pd.DataFrame, right: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if left_columns == right_columns:
        return (left_indexed.reindex(columns=left_columns), right_indexed.reindex(columns=right_columns))
    if len(left_columns) == 1 and left_columns[0] not in right_columns:
        target_columns = right_columns
        return (_broadcast_single_column_frame(left_indexed, target_columns=target_columns), right_indexed.reindex(columns=target_columns))
    if len(right_columns) == 1 and right_columns[0] not in left_columns:
        target_columns = left_columns
        return (left_indexed.reindex(columns=target_columns), _broadcast_single_column_frame(right_indexed, target_columns=target_columns))
    common_columns = [column for column in left_columns if column in right_columns]
    if common_columns:
        return (left_indexed.reindex(columns=common_columns), right_indexed.reindex(columns=common_columns))
    return left_indexed.align(right_indexed, join='outer', axis=1, fill_value=np.nan)

def _broadcast_single_column_frame(frame: pd.DataFrame, *, target_columns: list[str]) -> pd.DataFrame:
    if len(frame.columns) != 1:
        raise ValueError('broadcast_single_column_frame requires exactly one column')
    series = frame.iloc[:, 0]
    return pd.DataFrame({column: series for column in target_columns}, index=frame.index).reindex(columns=target_columns)
...
def _safe_div(left: pd.DataFrame | float, right: pd.DataFrame | float) -> pd.DataFrame:
    if isinstance(left, pd.DataFrame) and isinstance(right, pd.DataFrame):
        ...
        out = pd.DataFrame(float(left), index=right.index, columns=right.columns).div(right.replace(0.0, np.nan))
    else:
...
def _kalman_beta_frame(left: pd.DataFrame, right: pd.DataFrame, *, process_noise: float, observation_noise: float) -> pd.DataFrame:
    aligned_left, aligned_right = left.align(right, join='outer')
