...
const TRACK_COLORS = {
  ...
  systematic_carry: "#f0b456",
  market_neutral_carry: "#f0b456",
};
...
async function refresh() {
  ...
  if (!experiments.some((row) => row.candidate_hash === state.selectedHash)) {
    const promoted = [...experiments].reverse().find((row) => row.promoted);
    state.selectedHash = promoted?.candidate_hash || experiments.at(-1)?.candidate_hash || null;
  }
...
function render() {
  ...
  if (!experiments.some((row) => row.candidate_hash === state.selectedHash)) {
    state.selectedHash = experiments.at(-1)?.candidate_hash || null;
  }
...
function renderScope(experiments) {
  ...
  if (experimentsSubtitle) {
    experimentsSubtitle.textContent =
      selectedRun
        ? `Showing experiments for ${selectedRun.agent_label} / ${selectedRun.run_label}. Run iteration and lineage row are both shown in the table.`
        : family === "all"
...
function renderSummary(experiments, runs) {
  ...
  const cards = [
    ...
    {
      label: "Promoted",
      value: `${experiments.filter((row) => row.promoted).length}`,
...
function renderRuns(runs) {
  ...
  runs.forEach((row) => {
    ...
    tr.innerHTML = `
      ...
      <td>${escapeHtml(row.agent_label || "unknown")}</td>
      ...
      <td>${row.best_candidate_hash ? `<a class="table-link" href="/experiments/${encodeURIComponent(row.best_candidate_hash)}">${escapeHtml(row.best_candidate_hash)}</a>` : "n/a"}</td>
...
function renderChart(experiments) {
  ...
  Object.entries(tracks).forEach(([track, rows]) => {
    ...
    rows.forEach((row) => {
      ...
      circle.setAttribute("r", row.promoted ? "6.5" : "4.5");
      circle.setAttribute("fill", row.passed ? color : "rgba(255,255,255,0.12)");
      circle.setAttribute("stroke", row.promoted ? "#fff" : "rgba(255,255,255,0.2)");
      circle.setAttribute("stroke-width", row.promoted ? "2" : "1.2");
      ...
      circle.addEventListener("click", () => {
        state.selectedHash = row.candidate_hash;
        renderTable(experiments);
        renderDetail(state.selectedHash);
      });
...
function renderTable(experiments) {
  ...
  [...experiments]
    ...
    .forEach((row) => {
      const tr = document.createElement("tr");
      if (row.candidate_hash === state.selectedHash) {
        tr.classList.add("selected");
      }
      tr.innerHTML = `
        <td>${escapeHtml(TRACK_LABELS[row.track] || row.track)}</td>
        ...
        <td class="${row.passed ? "status-pass" : "status-fail"}">${row.passed ? "pass" : "fail"}${row.promoted ? " / promoted" : ""}</td>
        <td><a class="table-link" href="/experiments/${encodeURIComponent(row.candidate_hash)}">More info</a></td>
...
async function renderDetail(candidateHash) {
  ...
  const response = await fetch(`/api/experiments/${candidateHash}`, { cache: "no-store" });
  if (!response.ok) {
  ...
  const policySweepBlock = renderDetailPolicySweep(summary, experiment.family || candidate.family);
    ...
    <div class="detail-grid">
      <div class="detail-block full-width detail-hero-block">
        <div class="detail-hero-top">
          <div>
            <h3>${escapeHtml(candidate.family || experiment.family)} <span class="mono">${escapeHtml(experiment.candidate_hash)}</span></h3>
            <div class="detail-actions">
              <a class="table-link" href="/experiments/${encodeURIComponent(experiment.candidate_hash)}">Open Full Experiment Page &rarr;</a>
            </div>
          </div>
          <div class="detail-status-badge ${experiment.passed ? "status-pass" : "status-fail"}">${experiment.passed ? "Passed" : "Failed"}${experiment.promoted ? " / Promoted" : ""}</div>
        </div>
        <div class="detail-hero-stats">
          ...
          <div class="detail-stat"><span class="detail-stat-label">Agent</span><span class="detail-stat-value">${escapeHtml(experiment.agent_label || "unknown")}</span></div>
      ...
      <div class="detail-block full-width">
        ...
        <p class="detail-copy">${escapeHtml(candidate.hypothesis || "No hypothesis recorded.")}</p>
        <div class="pill-row">
          ${(candidate.features || []).map((feature) => `<span class="pill">${escapeHtml(feature)}</span>`).join("")}
        </div>
...
function showTooltip(event, row, metricKey) {
  ...
  tooltip.innerHTML = `
    ...
    <div class="${row.passed ? "status-pass" : "status-fail"}">${row.passed ? "Passed" : "Failed"}${row.promoted ? " / Promoted" : ""}</div>
  `;
