...
class ReflectionRunner:
    ...
    async def run(
        ...
        *,
        ...
        content = await self.kimi.complete_text(
    ...
    def _fallback_frontmatter(self, evaluation_packet: dict[str, Any]) -> dict[str, Any]:
        ...
        candidate = dict(evaluation_packet.get("candidate") or {})
        ...
        failed_motif_signature = str(
            evaluation_packet.get("failed_motif_signature") or motif_signature(candidate)
        )
        feature_list = [str(feature) for feature in list(candidate.get("features") or [])]
