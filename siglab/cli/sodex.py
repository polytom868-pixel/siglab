...
def add_subparser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    preview_parser.add_argument('--kind', choices=['new-order', 'cancel-order', 'schedule-cancel', 'update-leverage', 'update-margin'], required=True)
...
async def run_valuechain_preflight(args: argparse.Namespace) -> None:
    ...
    report: dict[str, Any] = {'rpc_url': rpc_url, 'expected_chain_id': expected, 'source': 'https://sodex.com/documentation/user-guide/faq/how-do-i-add-the-valuechain-network', 'ready': False}
    try:
        async with httpx.AsyncClient(timeout=8.0) as client:
            response = await client.post(rpc_url, json={'jsonrpc': '2.0', 'id': 1, 'method': 'eth_chainId', 'params': []}, headers={'Content-Type': 'application/json'})
    ...
    style = 'success' if ready else 'error'
...
async def run_sodex_ws_probe(args: argparse.Namespace) -> None:
    ...
    report: dict[str, Any] = {'environment': args.environment, 'market': args.market, 'params': params, 'live_write': False, 'signed': False, 'ready': False}
    client = SoDEXWebSocketClient(environment=args.environment, market=args.market, idle_timeout_s=float(args.timeout_seconds), pong_timeout_s=min(5.0, float(args.timeout_seconds)), max_reconnects=0)
    try:
        ...
        try:
            ...
            if args.evidence_output:
                ...
                records = sodex_ws_evidence(update, observed_at=datetime.now(UTC).isoformat(), evidence_path=f'sodex/ws/{args.market}/{args.channel}')
...
def run_sodex_preview(args: argparse.Namespace) -> None:
    print_json(_sodex_preview_payload(args))

def _sodex_preview_payload(args: argparse.Namespace) -> dict[str, Any]:
    client = SoDEXSignedPerpsClient(api_key_name=None, account_id=int(args.account_id), signer=None, nonce_manager=None, environment='testnet', dry_run=True)
    if args.kind == 'new-order':
        order = perps_order_item(cl_ord_id=str(args.cl_ord_id), modifier=parse_sodex_enum(args.modifier, SODEX_MODIFIER_ALIASES, 'modifier'), side=parse_sodex_enum(args.side, SODEX_SIDE_ALIASES, 'side'), order_type=parse_sodex_enum(args.order_type, SODEX_ORDER_TYPE_ALIASES, 'order_type'), time_in_force=parse_sodex_enum(args.time_in_force, SODEX_TIME_IN_FORCE_ALIASES, 'time_in_force'), price=args.price, quantity=args.quantity, funds=args.funds, reduce_only=bool(args.reduce_only), position_side=parse_sodex_enum(args.position_side, SODEX_POSITION_SIDE_ALIASES, 'position_side'))
        request = client.new_order_request(symbol_id=int(args.symbol_id), orders=[order])
    elif args.kind == 'cancel-order':
        cancel = perps_cancel_item(symbol_id=int(args.symbol_id), order_id=args.order_id, cl_ord_id=args.orig_cl_ord_id)
        request = client.cancel_order_request(cancels=[cancel])
    elif args.kind == 'schedule-cancel':
        request = client.schedule_cancel_request(scheduled_timestamp=args.scheduled_timestamp)
    elif args.kind == 'update-margin':
        if args.amount is None:
            print('--amount is required for --kind update-margin', file=sys.stderr)
            raise SystemExit(1)
        request = client.update_margin_request(symbol_id=int(args.symbol_id), amount=str(args.amount))
    else:
        request = client.update_leverage_request(symbol_id=int(args.symbol_id), leverage=int(args.leverage), margin_mode=parse_sodex_enum(args.margin_mode, SODEX_MARGIN_MODE_ALIASES, 'margin_mode'))
    signature_input = build_signature_input(domain=request.domain, account_id=int(args.account_id), body=request.body, nonce=int(args.nonce))
    return {'method': request.method, 'path': request.path, 'domain': request.domain, 'weight': request.weight, 'canonical_body': canonical_json(http_body_from_action_payload(request.body)), 'canonical_signing_payload': canonical_json(request.body), 'signature_input': signature_input, 'signature': None, 'submitted': False}