...
class EvidenceStore:
    ...
    def append_many(self, records: Iterable[EvidenceRecord]) -> int:
        ...
        self.last_append_stats = {'records_seen': len(rows), 'records_appended': len(unique_rows), 'duplicates_skipped': len(rows) - len(unique_rows)}
    ...
    def query(self, *, entity: str | None=None, module: str | None=None, relation: str | None=None, limit: int=50) -> list[dict[str, Any]]:
        rows = self.load()
...
def etf_inflow_evidence(api_rows: Iterable[dict[str, Any]], *, etf_type: str, observed_at: str, evidence_path: str) -> list[EvidenceRecord]:
    records: list[EvidenceRecord] = []
    for row in api_rows:
        date = str(row.get('date') or '')
        if not date:
            continue
        for relation, attr_key in [('total_net_inflow', 'total_value_traded'), ('total_net_assets', 'cum_net_inflow')]:
            records.append(EvidenceRecord(source='sosovalue.etf_historical_inflow', observed_at=observed_at, timestamp=date, entity=etf_type, module='ETF', relation=relation, value=row.get(relation), confidence=0.95, evidence_path=evidence_path, attributes={attr_key: row.get(attr_key)}))
    return records

def news_evidence(rows: Iterable[dict[str, Any]], *, observed_at: str, evidence_path: str, module: str='Feeds', default_entity: str='market', source: str='sosovalue.featured_news') -> list[EvidenceRecord]:
    records: list[EvidenceRecord] = []
    for row in rows:
        ...
        entity = str(_first_of(row, ('currencyName', 'symbol', 'currencySymbol')) or _matched_currency_symbol(matched, preferred=default_entity) or default_entity)
        timestamp = _first_of(row, ('publishTime', 'publishedAt', 'createdAt', 'createTime', 'releaseTime'))
        records.append(EvidenceRecord(source=source, observed_at=observed_at, timestamp=str(timestamp) if timestamp is not None else None, entity=entity, module=module, relation='news_mention', value=title, confidence=0.75, evidence_path=evidence_path, attributes={'id': row.get('id'), 'url': _first_of(row, ('url', 'link', 'sourceLink')), 'summary': _first_of(row, ('summary', 'description')) or content.get('content'), 'author': row.get('author'), 'category': row.get('category'), 'matchedCurrencies': matched if isinstance(matched, list) else []}))
    return records

def sodex_ws_evidence(update: dict[str, Any], *, observed_at: str, evidence_path: str) -> list[EvidenceRecord]:
    ...
    for row in rows:
        ...
        records.append(EvidenceRecord(source='sodex.websocket', observed_at=observed_at, timestamp=str(timestamp) if timestamp is not None else observed_at, entity=symbol, module='SoDEX', relation=f'websocket_{channel or 'update'}', value=trade_value, confidence=0.8, evidence_path=evidence_path, attributes={'channel': channel, 'type': update_type, 'bid': bid, 'ask': ask, 'raw_keys': sorted(row.keys())}))
    return records
    ...
    for news in news_rows:
        ...
        for flow in flows:
            ...
            if entity not in {'', 'market'} and entity not in str(flow.get('entity') or '').lower():
                continue
            links.append({'source': 'siglab.evidence.link_feed_events_to_etf_flows', 'relation': 'feed_event_near_etf_flow', 'left_evidence_path': news.get('evidence_path'), 'right_evidence_path': flow.get('evidence_path'), 'entity': flow.get('entity'), 'feed_entity': news.get('entity'), 'day_gap': gap, 'confidence': 0.45 if entity == 'market' else 0.65, 'warning': 'temporal/categorical link only; not causal', 'feed_title': news.get('value'), 'flow_value': flow.get('value'), 'flow_date': flow.get('timestamp'), 'feed_timestamp': news.get('timestamp')})
    return links
...
def summarize_evidence(rows: Iterable[dict[str, Any]], links: Iterable[dict[str, Any]], *, top_links: int=10) -> dict[str, Any]:
    ...
    sorted_links = sorted(materialized_links, key=lambda link: (float(link.get('confidence') or 0.0), str(link.get('feed_timestamp') or '')), reverse=True)
    ...
    return {'record_count': len(materialized_rows), 'link_count': len(materialized_links), **counts, 'link_relation_counts': dict(sorted(Counter((str(link.get('relation') or '') for link in materialized_links)).items())), 'top_links': [{k: link.get(k) for k in top_link_keys} for link in sorted_links[:max(0, int(top_links))]]}
...
def _evidence_id(row: dict[str, Any]) -> str:
    ...
    if isinstance(value, bool):
        ...
        normalized_value = value
    stable_payload: dict[str, Any] = {'source': row.get('source'), 'entity': row.get('entity'), 'module': row.get('module'), 'relation': row.get('relation'), 'timestamp': row.get('timestamp'), 'value': normalized_value}
...
def _record_day(value: object) -> date | None:
    ...
    raw = str(value).strip()
    for parser in (lambda: datetime.fromisoformat(raw.replace('Z', '+00:00')), lambda: datetime.strptime(raw[:10], '%Y-%m-%d'), lambda: datetime.strptime(raw[:10], '%m/%d/%Y')):
        try:
            return parser().date()
        except ValueError:
            continue
