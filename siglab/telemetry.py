            ...
            tool_rows.append(
                {
                    ...
                    "had_error": bool((call.get("result") or {}).get("error")) if isinstance(call.get("result"), dict) else False,
                }
            )
    return {
        "trace_count": len(rows),
        "stage_counts": dict(sorted(_count(row["stage"] for row in rows).items())),
        "provider_counts": dict(sorted(_count(row["provider"] for row in rows if row.get("provider")).items())),
        "model_counts": dict(sorted(_count(row["model"] for row in rows if row.get("model")).items())),
        "tool_invocation_count": len(tool_rows),
        "tool_counts": dict(sorted(_count(row["name"] for row in tool_rows if row.get("name")).items())),
        "tool_latency_ms": {
...
def aggregate_provider_metrics_artifacts(metric_paths: Iterable[Path]) -> dict[str, Any]:
    ...
    return {
        ...
        "providers": dict(sorted(_count(snapshot.get("provider") for snapshot in snapshots if snapshot.get("provider")).items())),
        "models": dict(sorted(_count(snapshot.get("model") for snapshot in snapshots if snapshot.get("model")).items())),
