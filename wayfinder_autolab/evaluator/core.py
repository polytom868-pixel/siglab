...
from wayfinder_autolab.evaluator.score import serialize_stats, summarize_window_results
...
class ResearchEvaluator:
    ...
    async def evaluate(
        ...
        *,
        ...
        candidate = CandidateGraph.from_dict(candidate.canonical_dict())
        compiled = await compile_candidate(self.settings, self.provider, candidate)
        ...
        leverage_tiers = sorted(
            {1.0, min(2.0, candidate.risk.max_leverage), candidate.risk.max_leverage}
        )
        ...
        pair_policy_compatible = (
            ...
            or (
                candidate.family in {"perp_pair_trade_unlevered", "perp_pair_trade_levered"}
        ...
        passed, gate_reasons = evaluate_gates(candidate.track, summary)
        ...
        *,
            ...
            return {
                ...
                "visual_split": {
                    ...
                    "note": (
                        ...
                        "Autolab is using all available data for in-sample scoring."
                    ),
                    "ranges": (
                        [
                            {
            ...
            return {
                ...
                "visual_split": {
                    ...
                    "note": (
                        ...
                        "minimum 30-day audit slice. Autolab is reserving a single validation "
                        "holdout only."
                    ),
                    "ranges": [
                        {
        ...
        if len(selector_windows) >= 3:
            return {
                ...
                "visual_split": {
                    ...
                    "note": (
                        "Autolab now scores candidates on rolling train-validation chunks across "
        ...
        return {
            ...
            "visual_split": {
                ...
                "selector_uses_holdout": False,
                "note": (
                    "Autolab now scores on the in-sample slice only, monitors a validation holdout "
    ...
    def _pair_target_with_policy_sweep(
        ...
        *,
        ...
        if train_windows:
            for policy in self._pair_policy_candidates(
        ...
        candidate.params["gross_target"] = float(best_policy["gross_target"])
        candidate.params["max_gross_target"] = float(best_policy["max_gross_target"])
        candidate.params["entry_abs_score"] = float(best_policy["entry_abs_score"])
        candidate.params["exit_abs_score"] = float(best_policy["exit_abs_score"])
        candidate.params["flip_abs_score"] = float(best_policy["flip_abs_score"])
        candidate.params["max_holding_bars"] = int(best_policy["max_holding_bars"])
        candidate.params["cooldown_bars"] = int(best_policy["cooldown_bars"])
        candidate.params["signal_leverage_scale"] = float(best_policy["signal_leverage_scale"])
        candidate.params["min_abs_score"] = float(best_policy["entry_abs_score"])
