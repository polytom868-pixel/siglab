...
class TickerView:
    ...
    def price_change_pct(self) -> float:
        return float(self._raw.get('priceChangePercent', self._raw.get('price_change_pct', 0)) or 0)
...
class SymbolEntry:
    ...
    def from_ticker(cls: type[SymbolEntry], tv: TickerView) -> SymbolEntry:
        return cls(name=tv.symbol, symbol=tv.symbol, price=tv.last_price, change_pct=tv.price_change_pct, volume=tv.volume)
...
class OrderBookView:
    ...
    def from_dict(cls: type[OrderBookView], data: dict[str, Any], symbol: str) -> OrderBookView:
        return cls(bids=tuple(data.get('bids', [])), asks=tuple(data.get('asks', [])), symbol=symbol)
...
class PnlSnapshot:
    ...
    def from_dict(cls: type[PnlSnapshot], d: dict[str, Any]) -> PnlSnapshot:
        return cls(realized=float(d.get('realized_pnl', 0)), unrealized=float(d.get('unrealized_pnl', 0)), total=float(d.get('total_pnl', 0)), funding=float(d.get('total_funding_cost', 0)), open_count=int(d.get('open_position_count', 0)))
...
class RiskSnapshot:
    ...
    def from_dict(cls: type[RiskSnapshot], data: dict[str, Any]) -> RiskSnapshot:
        return cls(composite_score=data.get('composite_score'), sub_scores=dict(data.get('sub_scores', {})), strategy_count=int(data.get('strategy_count', 0)), max_drawdown=data.get('max_drawdown'), current_drawdown=data.get('current_drawdown'), recovery_periods=data.get('recovery_periods'), drawdown_history=tuple(data.get('drawdown_history', [])), correlation_matrix=data.get('correlation_matrix'), strategy_names=tuple(data.get('strategy_names', [])), alerts=tuple(data.get('alerts', [])))
