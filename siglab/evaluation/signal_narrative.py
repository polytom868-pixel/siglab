...
def _narrative_performance_summary(stats: dict) -> str:
    ...
    lines = ['=== Performance Summary ===', f'Total Return: {_fmt(total_ret)}' if total_ret is not None else 'Total Return: N/A', f'CAGR: {_fmt(cagr)}' if cagr is not None else 'CAGR: N/A', f'Max Drawdown: {_fmt(max_dd)}' if max_dd is not None else 'Max Drawdown: N/A', f'Calmar Ratio: {_fmt(calmar)}' if calmar is not None else 'Calmar Ratio: N/A']
    if liquidated:
...
def _narrative_drawdown_analysis(drawdown: dict) -> str:
    ...
    lines = ['=== Drawdown Analysis ===', f'Max drawdown: {_fmt(max_dd)} over {int(bars)} bar(s)' if max_dd is not None else 'Max drawdown: N/A', f'Dominant direction: {dominant_dir}' if dominant_dir else '']
    if long_frac is not None and short_frac is not None:
        lines.append(f'Position composition: {_fmt(long_frac * 100.0)}% long / {_fmt(short_frac * 100.0)}% short / {_fmt((flat_frac or 0.0) * 100.0)}% flat')
...
def _narrative_exemplar_trades(trades: dict) -> str:
    ...
    if winners:
        ...
        for i, trade in enumerate(winners[:3], 1):
            ...
            line = f'  {i}. {direction} | return: {_fmt(ret)} | bars: {_fmt(bars)}' if bars is not None else f'  {i}. {direction} | return: {_fmt(ret)}'
...
def _narrative_regime_context(regime: dict) -> str:
    ...
    regime_order = [('market_trend', 'Market Trend'), ('market_volatility', 'Volatility'), ('funding_level', 'Funding Level'), ('funding_dispersion', 'Funding Dispersion'), ('breadth', 'Breadth'), ('co_movement', 'Co-movement'), ('concentration', 'Concentration'), ('position_structure', 'Position Structure'), ('pair_volatility', 'Pair Volatility'), ('pair_correlation', 'Pair Correlation')]
...
def _narrative_gate_diagnostics(gates: dict) -> str:
    ...
    if regime_gates.get('configured'):
        ...
        lines.append(f'Regime gates active: {_fmt(gate_active * 100.0)}%' if gate_active is not None else 'Regime gates: configured')
