...
from siglab.workspace.indexes import load_jsonl
...
def suggest_feature_set(
    *,
    ...
    for row in _catalog_rows(workspace_root):
        ...
        if score <= 0:
            score = 0.1
        specs.append((score, row))
    specs.sort(key=lambda item: (item[0], str(item[1].get("name") or "")), reverse=True)
    selected: list[dict[str, Any]] = []
    seen_subkind: set[str] = set()
    for _score, row in specs:
