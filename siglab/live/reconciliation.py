...
class ReconciliationEngine:
    ...
    def __init__(self, divergence_threshold: float | None=None) -> None:
        self.divergence_threshold = divergence_threshold if divergence_threshold is not None else DEFAULT_DIVERGENCE_WARNING_THRESHOLD
    ...
    def compare(self, backtest_pnl: pd.Series, paper_pnl: pd.Series) -> dict[str, Any]:
        ...
        if len(common_idx) < 2:
            return {'overlapping_periods': len(common_idx), 'correlation': None, 'tracking_error': None, 'bias': None, 'divergence_warning': False, 'start_date': None, 'end_date': None, 'note': 'Insufficient overlapping periods for reconciliation'}
        ...
        return {'overlapping_periods': len(common_idx), 'correlation': correlation, 'tracking_error': round(tracking_error, 6) if tracking_error is not None else None, 'bias': round(bias, 6) if bias is not None else None, 'divergence_warning': divergence_warning, 'start_date': start_date, 'end_date': end_date, 'divergence_threshold': self.divergence_threshold}
