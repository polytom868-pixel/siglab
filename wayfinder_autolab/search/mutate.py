...
from wayfinder_autolab.feature_dsl import is_valid_feature_expression, load_feature_spec
...
from wayfinder_autolab.search.select import rank_deterministic_candidates
from wayfinder_autolab.settings import AutolabSettings
from wayfinder_autolab.track_registry import canonical_track_name, storage_track_name
...
DIRECTIONAL_PERPS_LOOKBACK_DAYS = 365
...
class CandidateMutator:
    def __init__(self, settings: AutolabSettings, kimi: KimiClient) -> None:
    ...
    def _merge_historical_seed_candidates(
        self,
        *,
        rows: list[CandidateGraph],
        track: str,
        family_scope: set[str] | None,
    ) -> list[CandidateGraph]:
        merged: dict[str, CandidateGraph] = {candidate.family: candidate for candidate in rows}
        ordered_families: list[str] = []
        for candidate in self._historical_seed_candidates(track=track, family_scope=family_scope):
            merged[candidate.family] = candidate
            if candidate.family not in ordered_families:
                ordered_families.append(candidate.family)
        for candidate in rows:
            if candidate.family not in ordered_families:
                ordered_families.append(candidate.family)
        return [merged[family_name] for family_name in ordered_families if family_name in merged]

    def _historical_seed_candidates(
        self,
        *,
        track: str,
        family_scope: set[str] | None,
        ...
        artifact_root = Path(getattr(self.settings, "artifact_dir", self.settings.root_dir / "artifacts"))
        search_dirs = [artifact_root / storage_track]
        search_dirs.extend(
            sorted(
                (
                    backup_dir / "artifacts" / storage_track
                    for backup_dir in (self.settings.root_dir / "backups").glob("relaunch_*")
                ),
                reverse=True,
            )
        )
        best_by_family: dict[str, tuple[float, float, CandidateGraph]] = {}
        for directory in search_dirs:
            ...
            for path in directory.glob("*.json"):
                ...
                family_name = str(candidate_payload.get("family") or "")
                if not family_name:
                    continue
                if canonical_track_name(str(candidate_payload.get("track"))) != track:
                    continue
                ...
                if not self._historical_seedworthy(summary):
                    continue
                try:
                    candidate = CandidateGraph.from_dict(candidate_payload)
                except Exception:  # noqa: BLE001
                    continue
                ...
                if current is None or (quality, freshness) > (current[0], current[1]):
                    best_by_family[family_name] = (quality, freshness, candidate)
        ranked = sorted(
            best_by_family.values(),
            key=lambda item: (item[0], item[1]),
            reverse=True,
        )
        return [candidate for _quality, _freshness, candidate in ranked]
    ...
    async def propose(
        ...
        *,
        ...
        llm_tools: list[KimiTool] | None = None,
        deterministic_recent_rows: list[dict[str, Any]] | None = None,
        deterministic_seed_candidates: list[CandidateGraph] | None = None,
    ) -> list[CandidateGraph]:
        ...
        proposals: list[CandidateGraph] = []
        if not skip_llm and self.kimi.is_configured:
        ...
        validated: list[CandidateGraph] = []
        seen: set[str] = set()
        for candidate in proposals:
            fixed = self._validate_candidate(
                candidate=candidate,
                track=track,
                allowed_families=allowed_families,
                allowed_features_by_family=allowed_features_by_family,
            ...
            candidate_hash = fixed.strategy_hash()
            if candidate_hash in seen or candidate_hash in exclude_hashes:
                continue
    ...
    async def _llm_proposals(
        ...
        *,
        ...
        user_payload = {
            "task": "Return exactly 1 candidate",
            "track": track,
            "allowed_families": allowed_families,
            ...
            "preferred_pair_universes": PAIR_UNIVERSES if track == "directional_perps" else [],
            "preferred_cross_sectional_universes": (
                CROSS_SECTIONAL_UNIVERSES if track == "directional_perps" else []
            ),
            ...
            "evaluation_policy": {
                "selector": (
                    "Autolab scores candidates on rolling validation chunks across the pre-audit "
                ...
                "history": (
                    ...
                    "candidates that remain stable if the realized history is materially shorter, "
            ...
            "rules": [
                ...
                "Use inspect_pre_audit_candidate when you need to inspect a stored candidate's filtered pre-audit trade episodes, pre-audit diagnostics, or parent-child lineage before proposing a mutation",
                ...
                "Use probe_candidate_gate_impact when adding, tightening, or questioning regime_gates; confirm on train-only data that the gates materially filter bars and improve train behavior instead of staying open almost all the time",
                "Use compare_intended_vs_frozen_candidate on recent misses when you need to know whether the idea was wrong or whether normalization and the sweep materially changed the thing that actually ran",
                ...
                "Read last_five_runs[].parent_delta, bottlenecks, and sweep_drift to see whether the run is actually improving or just surviving by trading less or by letting the sweep rewrite the candidate",
                "If the run seems stuck, mixed, or family selection is unclear, use summarize_experiment_frontier plus inspect_pre_audit_candidate to separate family-level edge from single-candidate noise before mutating again",
                ...
                "When proposing more than two regime gates, or when the thesis relies on gates being critical, verify the gates with probe_candidate_gate_impact before trusting the idea",
                "If recent misses show material sweep drift or dropped gates, use compare_intended_vs_frozen_candidate before mutating the parent again",
                "Do not jump straight to a new idea. First reconcile the strongest positive anchors, the outstanding runs, and the recurring failure modes into a coherent diagnosis, then propose a candidate that explicitly addresses that diagnosis",
                "For pair-trade formulas, treat the two legs symmetrically as asset_1 and asset_2 rather than assuming a privileged leg",
                "For pair-trade candidates, choose an explicit trade_style from reversion, pullback, continuation, breakout, or hybrid",
                "For pair-trade candidates, prefer interpretable signals such as relative momentum, spread dislocation, funding divergence, carry normalized by volatility, regime filters, dynamic residuals, hedge-ratio drift, mean-reversion speed, and compression or expansion state",
                "For pair-trade candidates, build the idea in layers: thesis, core signal, regime gates, exit or holding logic, and cooldown when needed",
                "For pair-trade candidates, use regime_gates.entry to express true regime gating; additive features alone do not create a hard gate",
                ...
                "For pair-trade candidates, you may choose from the preferred pair universes and tune gross_target, max_gross_target, signal_leverage_scale, entry_abs_score, exit_abs_score, flip_abs_score, max_holding_bars, and cooldown_bars",
                "For directional candidates, think in terms of trend, breakout, pullback, reversal, breadth, and funding overlays across many assets rather than pair-ratio logic",
                "For basket-neutral candidates, think in terms of long basket versus short basket, relative dislocation, dispersion compression, hedge-ratio drift, and cross-sectional carry imbalance",
                "For carry candidates, think in terms of carry level, carry-to-vol, carry dispersion, funding stability, and avoiding carry flip risk",
                ...
                "Remember that the evaluator only does a local policy sweep. A good candidate should still make sense before the sweep; do not rely on the sweep to rescue a weak thesis",
                "Prefer realistic trade frequency and holding behavior; do not produce zero-trade, near-flat, or constant-flip candidates unless you can justify the sparse activity economically",
                "When losses cluster in one regime, suppress or gate that regime rather than stacking more entry signals",
                "Prefer candidates that remain robust if the realized history is shorter than the requested lookback",
                "Optimize for selector-window stability and validation robustness, not for the untouched final audit block",
                "Only emit these top-level keys per candidate: track, family, hypothesis, neutrality_basis, features, universe, risk, regime_gates, params",
        ...
        try:
            payload = await self.kimi.complete_json_with_tools(
                system_prompt=system_prompt,
                user_prompt=user_prompt,
                tools=llm_tools,
                max_tokens=min(self.settings.kimi_max_tokens, 4000),
                timeout_s=max(self.settings.kimi_timeout_s, 120.0),
                max_tool_rounds=max(25, self.settings.kimi_max_tool_rounds),
            )
            ...
            self.last_llm_trace = {
                ...
                "trace": dict(self.kimi.last_trace or {}),
        ...
        rows = self._extract_llm_candidate_rows(payload)[:1]
        if self.last_llm_trace is not None:
            self.last_llm_trace["candidate_count"] = len(rows)
        return [CandidateGraph.from_dict(row) for row in rows if isinstance(row, dict)]

    def _extract_llm_candidate_rows(self, payload: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        # Kimi sometimes returns the single candidate object directly instead of wrapping it.
        top_level_keys = {
            "track",
            "family",
            "hypothesis",
    ...
    def _formula_examples(self, track: str, family: str) -> list[str]:
        if track == "directional_perps" and family in PAIR_TRADE_FAMILIES:
            return [
                "sub(pct_change(asset_1_price,24), pct_change(asset_2_price,24))",
        ...
        if track == "directional_perps" and family in {
            BASKET_NEUTRAL_UNLEVERED_FAMILY,
            BASKET_NEUTRAL_LEVERED_FAMILY,
        }:
            return [
        ...
        if track == "directional_perps" and family == MULTI_ASSET_CARRY_FAMILY:
            return [
                "relative_carry_z_72h",
        ...
        if track == "directional_perps" and family in PAIR_TRADE_FAMILIES:
            return [
                ...
                },
                {
                    ...
                    "why": "Promotes setups where price leadership and carry pressure point the same way.",
                },
                {
    ...
    def _deterministic_variants(
        ...
        seed_by_family = {
            ...
            for candidate in self.load_seed_candidates(track, family=family)
        }
        ...
        if track == "directional_perps" and parent.family == "perp_multi_asset_decision":
            def _append(payload: dict[str, Any]) -> None:
                ...
                variants.append(CandidateGraph.from_dict(normalized))
            ...
            def _append_cross_sectional(payload: dict[str, Any], symbols: list[str]) -> None:
                ...
                normalized["universe"]["lookback_days"] = DIRECTIONAL_PERPS_LOOKBACK_DAYS
                _append(normalized)
            ...
            for pair_family in pair_family_order:
                pair_seed_candidate = seed_by_family.get(pair_family)
                if pair_seed_candidate is None:
                    continue
                pair_seed = pair_seed_candidate.canonical_dict()
            ...
            for family_name in family_order:
                ...
                if extra_seed is None:
                    continue
                variants.append(CandidateGraph.from_dict(extra_seed.canonical_dict()))
        ...
        elif track == "directional_perps" and parent.family in PAIR_TRADE_FAMILIES:
            ...
            multi_seed_candidate = seed_by_family.get("perp_multi_asset_decision")
            if multi_seed_candidate is not None:
                variants.append(CandidateGraph.from_dict(multi_seed_candidate.canonical_dict()))
            ...
            def _with_cross_sectional_universe(payload: dict[str, Any], symbols: list[str]) -> CandidateGraph:
                normalized = copy.deepcopy(payload)
                max_symbols = int(normalized["universe"].get("max_symbols", len(symbols) or 4))
                normalized["universe"]["basis_groups"] = list(
                    _trim_symbols(symbols, max_symbols=max_symbols)
                ...
                normalized["universe"]["lookback_days"] = DIRECTIONAL_PERPS_LOOKBACK_DAYS
                return CandidateGraph.from_dict(normalized)
            ...
            variants.append(CandidateGraph.from_dict(third))
            ...
            if parent.family == "perp_multi_asset_decision":
                ...
                fourth["params"]["min_abs_score"] = 0.2
            variants.append(CandidateGraph.from_dict(fourth))
        else:
            first = copy.deepcopy(base)
    ...
    def _normalize_regime_gates(
        ...
        *,
        ...
        execution_profile = self._family_execution_profile(candidate.track, candidate.family)
        if execution_profile not in {"ranked_directional", "basket_neutral_spread", "ranked_carry", "basis_hedged"}:
            return {}

        payload = dict(candidate.regime_gates or {})
    ...
    def _validate_candidate(
        ...
        *,
        ...
        if candidate.family not in allowed_families:
            raise ValueError(f"Unsupported family for track {track}: {candidate.family}")
        ...
        family_allowed_features = set(allowed_features_by_family.get(candidate.family) or [])
        cleaned_features = []
        ...
        if not cleaned_features:
            cleaned_features = list(family_defaults[candidate.family]["features"])
        candidate.features = cleaned_features
        candidate.regime_gates = self._normalize_regime_gates(
            candidate=candidate,
        ...
        candidate.universe.max_symbols = max(1, min(candidate.universe.max_symbols, 8))
        candidate.universe.lookback_days = max(
            21, min(candidate.universe.lookback_days, DIRECTIONAL_PERPS_LOOKBACK_DAYS)
        )
        candidate.risk.max_leverage = max(1.0, min(candidate.risk.max_leverage, 3.0))
        candidate.risk.rebalance_threshold = max(
            0.0, min(candidate.risk.rebalance_threshold, 0.25)
        )

        if track == "directional_perps":
            # Keep directional-perp experiments on a fixed horizon so
            # comparisons and charts are apples-to-apples across generations.
            candidate.universe.lookback_days = DIRECTIONAL_PERPS_LOOKBACK_DAYS
            execution_profile = self._family_execution_profile(track, candidate.family)
            candidate.params["min_abs_score"] = max(
                0.0,
                min(1.5, float(candidate.params.get("min_abs_score", 0.0))),
            )
            if candidate.family in PAIR_TRADE_FAMILIES:
                trade_style = str(candidate.params.get("trade_style") or "").strip().lower()
                if trade_style not in {
                    ...
                    trade_style = "hybrid"
                candidate.params["trade_style"] = trade_style
                candidate.risk.max_asset_weight = max(
                    0.10,
                    min(
                        candidate.risk.max_asset_weight,
                        0.5 if candidate.family == UNLEVERED_PAIR_FAMILY else 2.0,
                    ),
                )
                candidate.params["gross_target"] = max(
                    0.5,
                    min(1.5, float(candidate.params.get("gross_target", 1.0))),
                )
                max_gross_cap = 1.0 if candidate.family == UNLEVERED_PAIR_FAMILY else 3.0
                candidate.params["max_gross_target"] = max(
                    candidate.params["gross_target"],
                    min(
                        max_gross_cap,
                        float(
                ...
                candidate.params["signal_leverage_scale"] = max(
                    0.25,
                    min(3.0, float(candidate.params.get("signal_leverage_scale", 0.75))),
                )
                entry_abs_score = max(
                    0.0,
                    min(
                        ...
                        float(
                            candidate.params.get(
                                "entry_abs_score",
                                candidate.params.get("min_abs_score", 0.0),
                            )
                        ),
                    ),
                )
                candidate.params["entry_abs_score"] = entry_abs_score
                candidate.params["min_abs_score"] = entry_abs_score
                candidate.params["exit_abs_score"] = max(
                    0.0,
                    min(
                        entry_abs_score,
                        float(candidate.params.get("exit_abs_score", entry_abs_score * 0.5)),
                    ),
                )
                candidate.params["flip_abs_score"] = max(
                    entry_abs_score,
                    min(
                        2.5,
                        float(candidate.params.get("flip_abs_score", entry_abs_score)),
                    ),
                )
                candidate.params["max_holding_bars"] = max(
                    0,
                    min(24 * 14, int(candidate.params.get("max_holding_bars", 0))),
                )
                candidate.params["cooldown_bars"] = max(
                    0,
                    min(24 * 7, int(candidate.params.get("cooldown_bars", 0))),
                )
                candidate.universe.max_symbols = 2
                pair_symbols = list(candidate.universe.basis_groups[:2] or PAIR_UNIVERSES[0])
                if len(pair_symbols) < 2:
                    pair_symbols = PAIR_UNIVERSES[0]
                candidate.universe.basis_groups = pair_symbols
                if candidate.family == UNLEVERED_PAIR_FAMILY:
                    candidate.params["gross_target"] = min(
                        1.0,
                        float(candidate.params["gross_target"]),
                    )
                    candidate.params["max_gross_target"] = 1.0
                    candidate.risk.max_leverage = 1.0
                else:
                    candidate.risk.max_leverage = max(
                        candidate.params["max_gross_target"],
                        candidate.risk.max_leverage,
                    )
                if len(candidate.universe.basis_groups) < 2:
                    candidate.universe.basis_groups = PAIR_UNIVERSES[0]
                return candidate
            ...
            if execution_profile in {"basket_neutral_spread", "ranked_carry"}:
                candidate.risk.max_asset_weight = max(
                    ...
                    min(
                        candidate.risk.max_asset_weight,
                        0.5 if candidate.family == BASKET_NEUTRAL_UNLEVERED_FAMILY else 1.0,
                    ),
                )
                candidate.params["gross_target"] = max(
                    0.4,
                    min(2.5, float(candidate.params.get("gross_target", 1.0))),
                )
                candidate.params["long_count"] = max(
                    1,
                    min(
                        int(candidate.params.get("long_count", 2)),
                        candidate.universe.max_symbols,
                    ),
                )
                candidate.params["short_count"] = max(
                    1,
                    min(
                        int(candidate.params.get("short_count", 2)),
                        candidate.universe.max_symbols,
                    ),
                )
                candidate.params["long_enabled"] = self._coerce_bool(
                    candidate.params.get("long_enabled", True),
                    default=True,
                )
                candidate.params["short_enabled"] = self._coerce_bool(
                    candidate.params.get("short_enabled", True),
                    default=True,
                )
                if candidate.family == BASKET_NEUTRAL_UNLEVERED_FAMILY:
                    candidate.risk.max_leverage = 1.0
                return candidate

            candidate.risk.max_asset_weight = max(
                0.05, min(candidate.risk.max_asset_weight, 0.5)
            )
            candidate.params["gross_target"] = max(
                0.5,
                min(1.5, float(candidate.params.get("gross_target", 1.0))),
            ...
            candidate.params["long_count"] = max(
                1,
                min(
                    int(candidate.params.get("long_count", 1)),
                    candidate.universe.max_symbols,
                ),
            )
            candidate.params["short_count"] = max(
                0,
                min(
                    int(candidate.params.get("short_count", 0)),
                    candidate.universe.max_symbols,
                ),
            )
            candidate.params["long_enabled"] = self._coerce_bool(
            ...
            candidate.params["selection_count"] = max(
                1,
                min(
                    int(candidate.params.get("selection_count", 2)),
                    candidate.universe.max_symbols,
                ),
            )
            candidate.params["gross_target"] = max(
                0.4,
                min(1.2, float(candidate.params.get("gross_target", 0.8))),
            )
            hedge_mode = str(candidate.params.get("hedge_mode", "none")).lower()
            if hedge_mode not in {"none", "perp"}:
                hedge_mode = "none"
            candidate.params["hedge_mode"] = hedge_mode
            candidate.params["hedge_ratio"] = max(
                0.0,
                min(1.5, float(candidate.params.get("hedge_ratio", 0.75))),
            )
        return candidate
