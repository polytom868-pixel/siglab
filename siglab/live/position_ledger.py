...
def compute_trade_pnl(fill_price: float, quantity: float, prior_position: float, prior_entry: float, side: str) -> float:
...
def update_position(side: str, quantity: float, fill_price: float, prior_qty: float, prior_entry: float) -> tuple[float, float]:
    ...
    if prior_qty == 0:
        ...
        new_entry = (abs(prior_qty) * prior_entry + quantity * fill_price) / abs(new_qty)
    else:
        new_entry = fill_price if new_qty != 0 else 0.0
    return (new_qty, new_entry if new_qty != 0 else 0.0)

def calculate_fill_price(kline_close: float, kline_high: float, kline_low: float, kline_open: float, side: str, limit_price: float, order_type: str='LIMIT') -> tuple[float, bool]:
...
def compute_avg_entry(prior_qty: float, prior_entry: float, add_qty: float, add_price: float) -> float:
