    ...
    def render(self) -> Text:
        ...
        for pos in self.positions:
            ...
            if mark > 0 and entry > 0 and qty != 0:
                u = qty * (mark - entry) if qty > 0 else abs(qty) * (entry - mark)
            if show_mark:
                result.append(f"  {sym:<16}", style=TEXT_SECONDARY)
                result.append(
                    f"{format_price(qty):>12}  {format_price(entry):>12}  {format_price(mark):>12}  ",
                    style=TEXT_PRIMARY,
                )
                ...
                result.append(
                    f"{format_price(qty):>10}  {format_price(entry):>10}  ",
                    style=TEXT_PRIMARY,
                )
                result.append_text(format_pnl(u))
            result.append("\n")
...
class AccountSummaryWidget(Static):
    ...
    def render(self) -> Text:
        ...
        if not self.pnl_data:
            ...
            return result
        r = float(self.pnl_data.get("realized_pnl", 0))
        u = float(self.pnl_data.get("unrealized_pnl", 0))
        t = float(self.pnl_data.get("total_pnl", 0))
        f = float(self.pnl_data.get("total_funding_cost", 0))
        ...
        for lbl, val in [
            ...
            result.append(f"  {lbl:<12}", style=TEXT_MUTED)
            result.append_text(format_pnl(val))
            result.append("\n")
...
class PnlChartWidget(Static):
    ...
    def render(self) -> Text:
        ...
        if not self.pnl_history:
            ...
            return result
        result.append("  ")
        result.append_text(sparkline_text(self.pnl_history, width=50))
        result.append("\n")
        lo, hi, cur = min(self.pnl_history), max(self.pnl_history), self.pnl_history[-1]
        for lbl, v in [("Low:", lo), ("High:", hi), ("Now:", cur)]:
            result.append(f"  {lbl:<6}", style=TEXT_MUTED)
            result.append_text(format_pnl(v))
        result.append("\n")
        return result
...
class OrderFormWidget(Static):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._side, self._order_type = "BUY", "MARKET"
        self._symbol = self._quantity = self._price = self._error = self._success = ""
    ...
    def render(self) -> Text:
        ...
        r.append(
            " BUY ",
            style=f"bold #000000 on {ACCENT_GREEN}"
            if self._side == "BUY"
            else ACCENT_GREEN,
        )
        r.append(" ")
        r.append(
            " SELL ",
            style=f"bold #000000 on {ERROR_RED}" if self._side == "SELL" else ERROR_RED,
        )
        r.append("\n")
        r.append("  Type:    ")
        r.append(
            " MARKET ",
            style=f"bold #000000 on {INFO_BLUE}"
        ...
        if self._order_type == "LIMIT":
            r.append(f"  Price:   {self._price or '—'}\n", style=TEXT_SECONDARY)
        if self._error:
            r.append(f"  X {self._error}\n", style=ERROR_RED)
        if self._success:
            r.append(f"  V {self._success}\n", style=ACCENT_GREEN)
        r.append("\n")
        r.append(
    ...
    def _set_msg(self, err: str, suc: str) -> None:
...
class OrderHistoryWidget(Static):
    ...
    def render(self) -> Text:
        ...
        for o in self.orders[:50]:
            ...
            fp = o.get("fill_price")
            st = str(o.get("status", "?"))
            s_st = order_status_style(st)
            sd_st = side_style(sd)
            qs = compact_qty(qty)
            if show_price:
                result.append(f"  {ts:<11}", style=TEXT_MUTED)
                result.append(f" {sd:<5}", style=sd_st)
                result.append(f" {ot:<7}", style=TEXT_SECONDARY)
                result.append(f" {sym:<13}", style=TEXT_PRIMARY)
                result.append(f" {qs:>9}", style=TEXT_PRIMARY)
                if fp is not None:
                    result.append(f" {format_price(float(fp)):>9}", style=ACCENT_GREEN)
                elif price > 0:
                ...
                result.append(f"  {ts:<10}", style=TEXT_MUTED)
                result.append(f" {sd:<5}", style=sd_st)
                result.append(f" {ot:<6}", style=TEXT_SECONDARY)
                result.append(f" {truncate(sym, 9):<9}", style=TEXT_PRIMARY)
                result.append(f" {qs:>8}", style=TEXT_PRIMARY)
                result.append(f" {st}", style=s_st)
            result.append("\n")
        return result
...
class PaperScreen(BaseScreen):
    ...
    async def _find_existing_session(self, name: str) -> dict[str, Any] | None:
        ...
        ms = [
            s
            for s in (await self._api.list_paper_sessions()).get("sessions", [])
            if s.get("name") == name
        ]
        return (
            cast(dict[str, Any], sorted(ms, key=lambda s: s.get("created_at", 0))[-1])
            if ms
            else None
        )
    ...
    async def _init_session(self) -> None:
        ...
        try:
            try:
                ...
                if ex:
                    self.session_id = ex["session_id"]
                    self.session_name = ex.get("name", "tui-session")
                    self.is_loading, self.status_text = (
                        False,
    ...
    async def _fetch_data(self) -> None:
        ...
        try:
            ...
            self._upd_pos(data.get("position", []))
    ...
    def _upd_pos(self, positions: list[dict[str, Any]]) -> None:
        safe_query(
            ...
            lambda w: (
                setattr(w, "positions", positions),
                setattr(w, "mark_prices", self._mark_prices),
            ),
        )
    ...
    def _upd_pnl(self, pnl_data: dict[str, Any]) -> None:
        safe_query(
            ...
            lambda w: (
                setattr(w, "pnl_data", pnl_data),
                setattr(w, "session_name", self.session_name),
            ),
        )
        self._pnl_history.append(float(pnl_data.get("total_pnl", 0)))
    ...
    async def _place_order(self, params: dict[str, str]) -> None:
        ...
        if not self.session_id:
            ...
            return
        try:
            od = await self._api.place_paper_order(
                self.session_id,
            ...
            safe_query(
                ...
                "#order-form",
                OrderFormWidget,
                lambda w, m=str(exc)[:80]: w.show_error(m),
            )
            logger.warning("Order placement error: %s", exc)

    def action_focus_symbol(self) -> None:
        self._push_txt("symbol", "Enter symbol (e.g. BTC-USD):")
    ...
    def action_focus_qty(self) -> None:
        self._push_txt("quantity", "Enter quantity:")

    def action_focus_price(self) -> None:
        self._push_txt("price", "Enter limit price:")
    ...
    async def action_cancel_order(self) -> None:
        ...
        if not oo:
            ...
            return
        if len(oo) == 1:
            await self._do_cancel(oo[0]["order_id"])
        else:
            self.app.push_screen(_CancelOrderScreen(oo), callback=self._on_cancel_res)

    def _on_cancel_res(self, order_id: str | None) -> None:
        if order_id:
            self.call_after_refresh(self._do_cancel, order_id)
    ...
    def _on_txt_res(self, result: tuple[str, str] | None) -> None:
        ...
        if m:
            safe_query(self, "#order-form", OrderFormWidget, lambda w: getattr(w, m)(v))
...
class _TextInputScreen(Screen[tuple[str, str] | None]):
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        v = event.value.strip()
        self.dismiss((self._field, v) if v else None)
...
class _CancelOrderScreen(Screen[str | None]):
    ...
    def compose(self) -> ComposeResult:
        ls = [
            f"{i + 1}) {str(o.get('order_id', '?'))[:8]}... {o.get('side', '?')} {o.get('quantity', 0)} {o.get('symbol', '?')}"
...
def _correlation_color(value: float) -> str:
    return ERROR_RED if value >= 0.7 else WARNING_YELLOW if value >= 0.4 else TEXT_MUTED
...
class RiskGaugeWidget(Static):
    ...
    def render(self) -> Text:
        ...
        pct = int(sc * 100)
        color = gauge_color(sc)
        result.append("\n  ")
        result.append(bar_gauge(sc, width=24), style=f"bold {color}")
        ...
        for key, lbl in {
            ...
            if val is not None:
                vc = gauge_color(val)
                result.append(f"  {lbl:<12}", style=TEXT_SECONDARY)
                result.append(bar_gauge(val, width=10), style=vc)
                result.append(f" {val:.2f}\n", style=TEXT_PRIMARY)
            else:
                result.append(f"  {lbl:<12}", style=TEXT_SECONDARY)
                result.append(chr(9617) * 10, style=BORDER_DIM)
                result.append(" --\n", style=TEXT_MUTED)
...
class DrawdownSparklineWidget(Static):
    ...
    def render(self) -> Text:
        ...
        if not self.drawdown_history:
            ...
            result.append(chr(9472) * 30, style=BORDER_DIM)
            result.append("\n")
            return result
        vals = [-v for v in self.drawdown_history]
        avail = getattr(self.size, "width", 80) or 80
        cw = max(20, min(avail - 6, min(60, len(vals))))
        result.append("  ")
        result.append_text(sparkline_text(vals, width=cw, bearish_color=ERROR_RED))
        result.append("\n\n")
        ...
        result.append(
            f"{rp} periods" if rp is not None else "in progress",
            style=ACCENT_GREEN if rp is not None else WARNING_YELLOW,
        )
        result.append("\n")
        return result
...
class CorrelationHeatmapWidget(Static):
    ...
    def render(self) -> Text:
        ...
        for name in names[:sn]:
            result.append(f"{name[:mnl].rjust(mnl)} ", style=INFO_BLUE)
        result.append("\n")
        for i in range(min(n, sn)):
            ...
            for j in range(min(n, sn)):
                ...
                result.append(
                    f"{_correlation_block(val)}{val:.2f}".rjust(mnl) + " ",
                    style=_correlation_color(val) if i != j else TEXT_SECONDARY,
                )
            result.append("\n")
        if sn < n:
            result.append(f"  ... +{n - sn} more strategies\n", style=TEXT_MUTED)
        result.append("\n  Legend: ")
        result.append(chr(9608), style=TEXT_SECONDARY)
        result.append("=1.0 ", style=TEXT_MUTED)
        result.append(chr(9619), style=ERROR_RED)
        result.append(">=0.7 ", style=TEXT_MUTED)
        result.append(chr(9618), style=WARNING_YELLOW)
        result.append(">=0.4 ", style=TEXT_MUTED)
        result.append(chr(9617), style=TEXT_MUTED)
        result.append(">=0.1 ", style=TEXT_MUTED)
        result.append(chr(183), style=TEXT_MUTED)
        result.append("<0.1\n", style=TEXT_MUTED)
        return result
...
class AlertStreamWidget(Static):
    ...
    def render(self) -> Text:
        ...
        for alert in self.alerts[:MAX_ALERTS_DISPLAY]:
            ...
            sev = str(alert.get("severity", "info")).upper()[:4]
            msg = str(alert.get("message", ""))
            metric = str(alert.get("metric", ""))
            result.append(f"  {ts} ", style=TEXT_MUTED)
            result.append(f"{sev:<5}", style=f"bold {severity_color(sev.lower())}")
            if metric:
...
class RiskScreen(BaseScreen):
    ...
    async def _on_ws_risk_update(self, msg: dict[str, Any]) -> None:
        try:
            ...
            safe_query(
                ...
                RiskGaugeWidget,
                lambda w: (
                    setattr(w, "composite_score", cs),
                    setattr(w, "strategy_count", msg.get("strategy_count", 0)),
                ),
            )
            safe_query(
                self,
    ...
    async def _fetch_risk_data(self) -> None:
        ...
        try:
            ...
            safe_query(
                ...
                RiskGaugeWidget,
                lambda w: (
                    setattr(w, "composite_score", data.get("composite_score")),
                    setattr(w, "sub_scores", data.get("sub_scores", {})),
                    setattr(w, "strategy_count", int(data.get("strategy_count", 0))),
                ),
            )
            safe_query(
                self,
                ...
                DrawdownSparklineWidget,
                lambda w: (
                    setattr(w, "drawdown_history", data.get("drawdown_history", [])),
                    setattr(w, "max_drawdown", data.get("max_drawdown")),
                    setattr(w, "current_drawdown", data.get("current_drawdown")),
                    setattr(w, "recovery_periods", data.get("recovery_periods")),
                ),
            )
            safe_query(
                self,
                ...
                CorrelationHeatmapWidget,
                lambda w: (
                    setattr(w, "matrix", data.get("correlation_matrix")),
                    setattr(w, "strategy_names", data.get("strategy_names", [])),
                ),
            )
    ...
    def _apply_alert_filter(self) -> None:
        fl = (
            ...
            if self._sev == "all"
                ...
                if str(a.get("severity", "")).lower() == self._sev
            ]
        )
        safe_query(
    ...
    def action_filter_alerts(self) -> None:
        cy = ["all", "critical", "warning", "info"]
        self._sev = cy[(cy.index(self._sev) + 1) % 4] if self._sev in cy else "all"
        self._apply_alert_filter()
        self.notify(title="Alert Filter", message=f"Showing: {self._sev}", timeout=2)
