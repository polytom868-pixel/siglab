...
class LineageStore:
    ...
    def _init_db(self) -> None:
        with self._connect() as connection:
            ...
            connection.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_experiment_events_candidate_hash
                ON experiment_events (candidate_hash)
                """
            )
            connection.execute(
                """
                CREATE TABLE IF NOT EXISTS promotions (
                    ...
                    support_reason TEXT,
                    metadata_json TEXT,
                    FOREIGN KEY(candidate_hash) REFERENCES experiments(candidate_hash)
                )
                """
            )
            connection.execute(
    ...
    def record(
        ...
        *,
        ...
        candidate_payload["track"] = (
            canonical_track_name(candidate_payload.get("track")) or candidate_payload.get("track")
        )
        ...
        with self._connect() as connection:
            ...
            connection.execute(
                ...
                INSERT OR REPLACE INTO experiments (
                    ...
                    summary_json,
                    artifact_path
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, COALESCE((SELECT promoted FROM experiments WHERE candidate_hash = ?), 0), ?, ?)
                """,
                (
                    evaluation["candidate_hash"],
                    recorded_at,
    ...
    def promote(self, candidate_hash: str) -> None:
        with self._connect() as connection:
            connection.execute(
                "UPDATE experiments SET promoted = 1 WHERE candidate_hash = ?",
                (candidate_hash,),
            )
            connection.execute(
                """
    ...
    def record_promotion(self, payload: dict[str, Any]) -> None:
    ...
    def promotion(self, candidate_hash: str) -> dict[str, Any] | None:
        with self._connect() as connection:
    ...
    def memory_packet(
        ...
        *,
        ...
        parent_assets = set(self._candidate_assets(parent_payload))
        ...
        diagnostics_by_hash = {
            str(row.get("candidate_hash")): self._row_diagnostic_snapshot(row)
    ...
    def list_rows(self, *, track: str | None, limit: int) -> list[dict[str, Any]]:
        query = (
            "SELECT created_at, track, family, candidate_hash, aggregate_score, passed, promoted "
            "FROM experiments "
        )
        params: tuple[Any, ...]
        if track:
    ...
    def clear_passed(self, *, track: str | None = None) -> dict[str, Any]:
        ...
        with self._connect() as connection:
            rows = connection.execute(
                f"""
                SELECT candidate_hash, artifact_path, promoted
                FROM experiments
                WHERE {where_clause}
            ...
            candidate_hashes = [str(row[0]) for row in rows]
            artifact_paths = [str(row[1]) for row in rows if row[1]]
            placeholders = ",".join("?" for _ in candidate_hashes)
            promotions_deleted = connection.execute(
                f"DELETE FROM promotions WHERE candidate_hash IN ({placeholders})",
                tuple(candidate_hashes),
            ).rowcount
            event_rows_deleted = connection.execute(
                f"DELETE FROM experiment_events WHERE candidate_hash IN ({placeholders})",
                tuple(candidate_hashes),
            ).rowcount
            ...
            experiments_deleted = connection.execute(
                f"DELETE FROM experiments WHERE candidate_hash IN ({placeholders})",
                tuple(candidate_hashes),
            ).rowcount
            connection.commit()
        ...
        return {
            ...
            "promotions_deleted": int(promotions_deleted),
    ...
    def dashboard_rows(
        ...
        *,
        ...
        for row in rows:
            candidate = self._candidate_payload(row[9])
            payload_rows.append(
                {
                    "event_id": int(row[0]),
                    "created_at": row[1],
                    ...
                    "feature_hash": self._feature_hash(candidate.get("features") or []),
    ...
    def experiment_detail(self, candidate_hash: str) -> dict[str, Any] | None:
        ...
        candidate = self._candidate_payload(row[8])
        return {
            "created_at": row[0],
            "track": canonical_track_name(row[1]) or row[1],
            "family": row[2],
            ...
            "feature_hash": self._feature_hash(candidate.get("features") or []),
            "promotion": self.promotion(candidate_hash),
        }
    ...
    def run_summaries(
        ...
        *,
        ...
        for row in rows:
            ...
            run_session_id = str(run_context.get("run_session_id") or f"legacy::{row['candidate_hash']}")
        ...
        for run_session_id, members in grouped.items():
            ...
            for row in ordered:
                ...
                promoted_count += int(bool(row.get("promoted")))
            ...
            summaries.append(
                {
                    ...
                    "best_candidate_hash": best.get("candidate_hash"),
    ...
    def _candidate_payload(self, raw_json: str) -> dict[str, Any]:
    ...
    def _experiment_row_payload(self, row: tuple[Any, ...]) -> dict[str, Any]:
        return {
            ...
            "passed": bool(row[5]),
            "promoted": bool(row[6]),
            "candidate": self._candidate_payload(row[7]),
    ...
    def _pareto_frontier(
        ...
        *,
        ...
        for row in rows:
            ...
            dominated = False
            for other in rows:
                if other["candidate_hash"] == row["candidate_hash"]:
                    continue
            ...
            frontier.append(
                {
                    ...
                    "assets": self._candidate_assets(row["candidate"]),
                }
            )
        frontier.sort(
            key=lambda row: (
    ...
    def _top_similar(
        ...
        *,
        ...
        for row in ranked[:limit]:
            diagnostic = diagnostics_by_hash.get(str(row.get("candidate_hash")) or "", {})
            payloads.append(
                {
                    "candidate_hash": row["candidate_hash"],
                    "family": row["family"],
                    ...
                    "features": list(row["candidate"].get("features") or [])[:8],
                    "assets": self._candidate_assets(row["candidate"]),
                    "maturity_bucket": self._maturity_bucket(row["candidate"].get("universe") or {}),
    ...
    def _candidate_similarity(
        ...
        *,
        ...
        score = 0.0
        if candidate.get("family") == parent_payload.get("family"):
            score += 5.0
        if candidate.get("neutrality_basis") == parent_payload.get("neutrality_basis"):
            score += 1.0

        other_assets = set(self._candidate_assets(candidate))
        score += float(len(parent_assets & other_assets)) * 1.5

        other_features = set(str(feature) for feature in candidate.get("features") or [])
        score += float(len(parent_features & other_features)) * 0.35

        if self._maturity_bucket(candidate.get("universe") or {}) == parent_maturity:
            score += 1.0
        if (candidate.get("params") or {}).get("hedge_mode") == (parent_payload.get("params") or {}).get("hedge_mode"):
            score += 0.75
        if bool(other.get("promoted")):
            score += 0.25
        return score
    ...
    def _coverage_summary(self, rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for row in rows:
            ...
            asset_counts.update(self._candidate_assets(row["candidate"]))
            feature_counts.update(str(feature) for feature in row["candidate"].get("features") or [])
    ...
    def _archetype_coverage(self, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for row in rows:
            trade_style = self._trade_style(row.get("candidate") or {})
            ...
            payloads.append(
                {
                    ...
                    "trade_style": diagnostic.get("trade_style"),
                    "features": list((row.get("candidate") or {}).get("features") or [])[:6],
                    "summary": {
    ...
    def _row_diagnostic_snapshot(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        policy = {
            ...
            **self._policy_snapshot(candidate),
        }
        ...
        return {
            "trade_style": self._trade_style(candidate),
            "policy": policy,
            "behavior_pack": behavior_pack,
            "regime_pack": regime_pack,
    ...
    def _parent_delta(
        ...
        *,
        ...
        if parent_row is None:
            return {}
        child_diag = diagnostics_by_hash.get(str(row.get("candidate_hash")) or "", {})
    ...
    def _policy_snapshot(self, candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    def _query_relevance(
        ...
        parent_tokens.update(_tokens(" ".join(self._candidate_assets(parent))))
    ...
    def _candidate_assets(self, candidate: dict[str, Any]) -> list[str]:
