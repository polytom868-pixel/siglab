...
class MarketDataProvider:
    ...
    def __init__(self, settings: SiglabConfig, lake: ParquetLake, *, config_path: str | None=None, sodex_feeds: SoDEXFeeds | None=None) -> None:
    ...
    def metrics_snapshot(self) -> dict[str, Any]:
        ...
        snap: dict[str, Any] = {'sosovalue': self.sosovalue.metrics_snapshot()}
    ...
    async def build_research_summary(self, track: str, parent: SignalSpec) -> dict[str, Any]:
        ...
        if track == 'yield_flows' and (not preferred_perp_symbols):
            preferred_perp_symbols = ['BTC', 'ETH', 'SOL', 'HYPE', 'DOGE']
        symbols = await self.discover_perp_symbols(preferred_perp_symbols, limit=min(max(parent.universe.max_symbols, 5), 5))
        ...
        if not perp_bundle['prices'].empty:
            ...
            pair_calibration = _pair_calibration_snapshot(prices=prices, funding=funding, symbols=symbols)
            if pair_calibration:
                summary['pair_calibration'] = pair_calibration
        if track == 'yield_flows':
            stable_universe = AssetUniverse(basis_groups=['USD'], chains=['arbitrum', 'base', 'plasma'], max_symbols=5, lookback_days=120, interval='1d', min_liquidity_usd=250000.0, min_volume_usd_24h=25000.0, min_days_to_expiry=10, max_days_to_expiry=180)
            rotation_universe = parent.universe
            if parent.family not in {'pt_yield_rotation', 'stable_pt_ladder'}:
                rotation_universe = AssetUniverse(basis_groups=['BTC', 'ETH', 'SOL'], chains=['arbitrum', 'base'], max_symbols=5, lookback_days=120, interval='1d', min_liquidity_usd=250000.0, min_volume_usd_24h=25000.0, min_days_to_expiry=10, max_days_to_expiry=180)
            lending_universe = parent.universe
            if parent.family != 'lending_carry_rotation':
                lending_universe = AssetUniverse(basis_groups=['ETH', 'BTC', 'SOL'], chains=['arbitrum', 'base', 'unichain'], max_symbols=5, lookback_days=90, interval='1h', min_liquidity_usd=250000.0, min_volume_usd_24h=25000.0, min_days_to_expiry=7, max_days_to_expiry=180)
            stable_markets, rotation_markets, lending_markets = await asyncio.gather(self.discover_stable_pt_markets(stable_universe, limit=min(stable_universe.max_symbols, 5)), self.discover_pt_markets(rotation_universe, limit=min(rotation_universe.max_symbols, 5)), self.discover_lending_markets(lending_universe, limit=min(parent.universe.max_symbols, 5)))
            summary['stable_pt_markets'] = [{'market': market['marketName'], 'chain_id': market['chainId'], 'fixed_apy': market['fixedApy'], 'underlying_apy': market['underlyingApy'], 'days_to_expiry': market['daysToExpiry']} for market in stable_markets[:5]]
            summary['pt_rotation_markets'] = [{'market': market['marketName'], 'chain_id': market['chainId'], 'fixed_apy': market['fixedApy'], 'underlying_apy': market['underlyingApy'], 'days_to_expiry': market['daysToExpiry'], 'hedge_symbol': market.get('hedgeSymbol')} for market in rotation_markets[:5]]
            summary['lending_markets'] = [{'market': market['marketLabel'], 'basis_symbol': market['basisSymbol'], 'venue': market['venue_name'], 'symbol': market['symbol'], 'net_supply_apr_now': market.get('net_supply_apr_now'), 'combined_net_supply_apr_now': market.get('combined_net_supply_apr_now'), 'util_now': market.get('util_now'), 'hedge_symbol': market.get('hedgeSymbol')} for market in lending_markets[:5]]
        sosovalue_etf: Any
        sosovalue_news: Any
        ...
        if isinstance(sosovalue_etf, list):
            summary['etf_inflow'] = [{'date': row.get('date'), 'total_net_inflow': row.get('total_net_inflow'), 'total_net_assets': row.get('total_net_assets')} for row in sosovalue_etf[:10]]
        if isinstance(sosovalue_news, list):
            summary['featured_news'] = [{'title': row.get('title'), 'source_link': row.get('source_link'), 'published_at': row.get('published_at')} for row in sosovalue_news[:5]]
    ...
    async def discover_perp_symbols(self, preferred_symbols: list[str], *, limit: int) -> list[str]:
        ...
        warm_key = self._warm_cache_key('perp_symbols', preferred_symbols=preferred_symbols, limit=limit)
    ...
    async def fetch_perp_bundle(self, *, symbols: list[str], lookback_days: int, interval: str) -> dict[str, Any]:
        ...
        cache_key = self._bundle_cache_key('perp_bundle', symbols=symbols, lookback_days=lookback_days, interval=interval)
        warm_key = self._warm_cache_key('perp_bundle', symbols=symbols, lookback_days=lookback_days, interval=interval)
        ...
        if warm_key in self._warm_cache:
            ...
            self._persist_bundle_frames(cache_key, prices=bundle['prices'], funding=bundle['funding'])
            return bundle
        if hasattr(self, '_fetch_perp_bundle_delta_lab'):
            bundle = await self._fetch_perp_bundle_delta_lab(symbols=symbols, lookback_days=lookback_days, interval=interval)
        else:
            bundle = await self._fetch_perp_bundle_sodex(symbols=symbols, lookback_days=lookback_days, interval=interval)
