...
def add_subparser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    start_parser = subparsers.add_parser('paper-start', help='Create a new paper trading session.')
    ...
    status_parser = subparsers.add_parser('paper-status', help='Show paper trading session status.')
    ...
    promote_parser = subparsers.add_parser('paper-promote', help='Check paper session promotion eligibility and promote if eligible.')
                ...
                for sym, klines in zip(open_symbols, results):
                    ...
                    if not klines.empty:
                        try:
                            ...
                            import logging
                            logging.getLogger(__name__).exception('process_klines failed for %s', sym)
        except Exception:
            import logging
...
async def run_paper_promote(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        sub_scores = {k: round(v, 4) for k, v in compute_sub_scores(metrics).items()}
        composite = compute_composite_score(metrics)
        eligible, reason = promotion_eligible(daily_metrics, threshold=threshold, consecutive_days=consecutive_days, min_trading_days=min_trading_days)
        result = {'promoted': eligible, 'reason': reason, 'composite_score': round(composite, 4), 'sub_scores': sub_scores, 'trade_count': metrics.get('trade_count', 0), 'trading_days': len(daily_metrics), 'threshold': threshold, 'consecutive_days_required': consecutive_days, 'min_trading_days_required': min_trading_days}
        print_json(result)
        if not eligible:
            raise SystemExit(1)
    except PaperClientError as exc:
        result = {'promoted': False, 'reason': str(exc), 'composite_score': 0.0, 'sub_scores': {}, 'trade_count': 0, 'trading_days': 0, 'threshold': args.threshold or DEFAULT_PROMOTION_THRESHOLD, 'consecutive_days_required': args.consecutive_days or DEFAULT_CONSECUTIVE_DAYS, 'min_trading_days_required': args.min_trading_days or DEFAULT_MIN_TRADING_DAYS}
        print_json(result)
        raise SystemExit(1)