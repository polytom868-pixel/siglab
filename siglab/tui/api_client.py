...
class TuiApiClient:
    ...
    @staticmethod
    async def _do_request(client: httpx.AsyncClient, method: str, path: str, **kwargs: Any) -> dict[str, Any]:
    ...
    async def get_strategies(self, track: str | None=None, family: str | None=None) -> dict[str, Any]:
    ...
    async def get_market_klines(self, symbol: str, interval: str='1h', limit: int=60) -> dict[str, Any]:
        """Fetch kline/candlestick data for a perp symbol."""
        return await self._get(f'/market/klines/{symbol}', params={'interval': interval, 'limit': limit})

    async def get_market_orderbook(self, symbol: str, limit: int=20) -> dict[str, Any]:
    ...
    async def place_paper_order(self, session_id: str, *, symbol: str, side: str, quantity: float, order_type: str='MARKET', price: float | None=None) -> dict[str, Any]:
        """Place a paper order."""
        body: dict[str, Any] = {'symbol': symbol, 'side': side, 'quantity': quantity, 'order_type': order_type}
        if price is not None:
            body['price'] = price
        return await self._post(f'/paper/sessions/{session_id}/orders', json=body)

    async def cancel_paper_order(self, session_id: str, order_id: str) -> dict[str, Any]:
        """Cancel a paper order."""
        return await self._request_with_retry('delete', f'/paper/sessions/{session_id}/orders/{order_id}')
    ...
    async def ws_subscribe_risk(self, callback: WsCallback) -> None:
        ...
        try:
            ...
            try:
                await ws.send(json.dumps({'action': 'subscribe', 'subscription_type': 'risk_score'}))
                async for raw in ws:
                    try:
                        msg = json.loads(raw)
