...
from siglab.models import SignalSpec, AssetUniverse
from siglab.settings import SiglabConfig
from siglab.track_registry import canonical_track_name
from sosovalue_paths.adapters.pendle_adapter import PendleAdapter
from sosovalue_paths.core.clients.DeltaLabClient import DeltaLabClient
from sosovalue_paths.core.config import CONFIG, load_config, set_config
...
class MarketDataProvider:
    def __init__(
        ...
        *,
        ...
        self.lake = lake
        load_config(config_path or settings.sosovalue_config_path, require_exists=True)
        if settings.sosovalue_api_key_override:
            config = dict(CONFIG)
            system = dict(config.get("system") or {})
            system["api_key"] = settings.sosovalue_api_key_override
