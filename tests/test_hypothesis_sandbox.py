...
from wayfinder_autolab.research.hypothesis import HypothesisSandbox
...
class HypothesisSandboxTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
        ...
        self.settings = AutolabSettings(
            root_dir=REPO_ROOT,
            wayfinder_config_path=root / "config.json",
            generated_strategy_dir=root / "generated_strategies",
            data_lake_dir=root / "lake",
            artifact_dir=root / "artifacts",
            live_dir=root / "live",
            lineage_db_path=root / "autolab_test.db",
        ...
        self.parent = CandidateGraph.from_dict(
            {
    ...
    async def test_probe_feature_forward_stats_reports_horizons_and_predictor_correlations(self) -> None:
        tool = self.sandbox.kimi_tools(track="directional_perps", parent=self.parent)[0]
    ...
    async def test_probe_candidate_gate_impact_reports_gated_vs_ungated_train_summary(self) -> None:
        tools = {
            tool.name: tool
            for tool in self.sandbox.kimi_tools(track="directional_perps", parent=self.parent)
        }
        tool = tools["probe_candidate_gate_impact"]
    ...
    async def test_compare_intended_vs_frozen_candidate_reports_drift_and_dropped_gates(self) -> None:
        tools = {
            tool.name: tool
            for tool in self.sandbox.kimi_tools(track="directional_perps", parent=self.parent)
        }
        tool = tools["compare_intended_vs_frozen_candidate"]
        lineage = LineageStore(self.settings.lineage_db_path)

        child = CandidateGraph.from_dict(
            {
                **self.parent.canonical_dict(),
                "hypothesis": "child candidate",
        ...
        llm_log = self.settings.artifact_dir / "llm_traces" / "directional_perps" / "intent.json"
        ...
        child_artifact = self.settings.artifact_dir / "directional_perps" / "intent_child.json"
        ...
        lineage.record(
            ...
            research_summary={
                ...
                "llm_tool_trace": {
                    ...
                    "trace": {"tool_names": ["probe_candidate_gate_impact"]},
                },
            },
            artifact_path=str(child_artifact),
        )

        result = await tool.handler({"candidate_hash": child.strategy_hash()})
    ...
    async def test_summarize_experiment_frontier_reports_family_level_positive_anchors(self) -> None:
        ...
        carry_seed = CandidateGraph.from_dict(
            {
                "track": "directional_perps",
        ...
        carry_child = CandidateGraph.from_dict(
            {
        ...
        directional_loser = CandidateGraph.from_dict(
            {
                "track": "directional_perps",
        ...
        def write_artifact(name: str, *, active_bar_fraction: float, bottlenecks: list[str]) -> Path:
            artifact_path = self.settings.artifact_dir / "directional_perps" / name
        ...
        lineage.record(
            evaluation={
                "candidate": carry_seed.canonical_dict(),
                "candidate_hash": carry_seed.strategy_hash(),
                "summary": {
                    "aggregate_score": 3.4,
        ...
        lineage.record(
            evaluation={
                "candidate": carry_child.canonical_dict(),
                "candidate_hash": carry_child.strategy_hash(),
                "summary": {
                    "aggregate_score": 3.9,
        ...
        lineage.record(
            evaluation={
                "candidate": directional_loser.canonical_dict(),
                "candidate_hash": directional_loser.strategy_hash(),
                "summary": {
                    "aggregate_score": -0.7,
        ...
        self.assertEqual(
            result["top_positive_anchors"][0]["candidate_hash"],
    ...
    async def test_inspect_pre_audit_candidate_filters_trade_episodes_and_hides_audit(self) -> None:
        tools = self.sandbox.kimi_tools(track="directional_perps", parent=self.parent)
        tool = next(tool for tool in tools if tool.name == "inspect_pre_audit_candidate")
        lineage = LineageStore(self.settings.lineage_db_path)
        ...
        parent_artifact = self.settings.artifact_dir / "directional_perps" / "parent.json"
        ...
        child_artifact = self.settings.artifact_dir / "directional_perps" / "child.json"
        child_artifact.write_text(
            json.dumps(
                {
                    "canonical_run": {
        ...
        self.assertEqual(result["lineage"]["parent"]["candidate_hash"], parent.strategy_hash())
