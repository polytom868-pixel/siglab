...
class SoDEXSignedPerpsClient(SoDEXPublicPerpsClient):
    ...
    def __init__(self, *, api_key_name: str | None, account_id: int | None, signer: SoDEXSigner | None, nonce_manager: SoDEXNonceManager | None, environment: str='mainnet', dry_run: bool=True, **kwargs: Any) -> None:
        super().__init__(base_url=kwargs.pop('base_url', 'https://mainnet-gw.sodex.dev/api/v1/perps') if environment == 'mainnet' else kwargs.pop('base_url', 'https://testnet-gw.sodex.dev/api/v1/perps'), **kwargs)
    ...
    def new_order_request(self, *, symbol_id: int, orders: list[OrderedDict[str, Any]]) -> SoDEXSignedRequest:
        ...
        return SoDEXSignedRequest(method='POST', path='/trade/orders', body=body, domain='futures', weight=_batch_order_weight(len(orders)))
    ...
    def update_leverage_request(self, *, symbol_id: int, leverage: int, margin_mode: int) -> SoDEXSignedRequest:
        ...
        return SoDEXSignedRequest(method='POST', path='/trade/leverage', body=perps_update_leverage_body(account_id=self.account_id, symbol_id=symbol_id, leverage=leverage, margin_mode=margin_mode), domain='futures', weight=1)
    ...
    def cancel_order_request(self, *, cancels: list[OrderedDict[str, Any]]) -> SoDEXSignedRequest:
        ...
        return SoDEXSignedRequest(method='DELETE', path='/trade/orders', body=perps_cancel_order_body(account_id=self.account_id, cancels=cancels), domain='futures', weight=_batch_order_weight(len(cancels)))
    ...
    def schedule_cancel_request(self, *, scheduled_timestamp: int | None=None) -> SoDEXSignedRequest:
        ...
        return SoDEXSignedRequest(method='POST', path='/trade/orders/schedule-cancel', body=perps_schedule_cancel_body(account_id=self.account_id, scheduled_timestamp=scheduled_timestamp), domain='futures', weight=1)
    ...
    def update_margin_request(self, *, symbol_id: int, amount: str) -> SoDEXSignedRequest:
        ...
        return SoDEXSignedRequest(method='POST', path='/trade/margin', body=perps_update_margin_body(account_id=self.account_id, symbol_id=symbol_id, amount=amount), domain='futures', weight=1)
    ...
    def prepare_signed_request(self, request: SoDEXSignedRequest) -> dict[str, Any]:
        ...
        signature_input = build_signature_input(domain=request.domain, account_id=self.account_id, body=request.body, nonce=nonce)
        signature = self.signer.sign_typed_payload(domain=request.domain, account_id=self.account_id, payload_hash=signature_input['payloadHash'], nonce=nonce)
        http_body = http_body_from_action_payload(request.body)
        return {'method': request.method.upper(), 'url': f'{self.base_url}/{request.path.lstrip('/')}', 'body': canonical_json(http_body), 'signing_payload': canonical_json(request.body), 'headers': build_signed_headers(api_key_name=self.api_key_name, signature=signature, nonce=nonce), 'signature_input': signature_input, 'weight': request.weight}

    async def send_signed_request(self, request: SoDEXSignedRequest) -> dict[str, Any]:
        prepared = self.prepare_signed_request(request)
        if self.dry_run:
            logger.warning('SoDEX signed client is in dry-run mode; skipping live write to %s %s (set dry_run=False to enable real writes)', prepared['method'], prepared['url'])
        ...
        try:
            response = await self._http().request(prepared['method'], prepared['url'], headers=prepared['headers'], content=prepared['body'], timeout=self.timeout_s)
    ...
    async def place_market_order(self, *, symbol_id: int, is_buy: bool, size: str, reduce_only: bool=False, cl_ord_id: str | None=None) -> dict[str, Any]:
        """Bridge method: compose a market order and submit via the signed path."""
        order = perps_order_item(cl_ord_id=cl_ord_id or f'w9_{uuid.uuid4().hex[:12]}', modifier=1, side=1 if is_buy else 2, order_type=2, time_in_force=1, quantity=str(size), reduce_only=reduce_only)
