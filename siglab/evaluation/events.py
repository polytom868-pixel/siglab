...
def classify_pt_market_state(
    *,
    ...
    inside_roll_window = (
        availability
        & days_to_expiry.gt(0.0)
        & days_to_expiry.le(float(roll_days_before_expiry))
    )
    ...
    eligible = (
        availability
        & within_maturity_range
        & days_to_expiry.gt(float(roll_days_before_expiry))
    )
    return {
...
def summarize_pt_universe(
    *,
    ...
    return {
        "eligible_market_count_min": int(eligible_counts.min())
        if not eligible_counts.empty
        else 0,
        "eligible_market_count_max": int(eligible_counts.max())
        if not eligible_counts.empty
        else 0,
    ...
    *,
    ...
    for index in range(1, len(pt_positions.index)):
        ...
        if not exited_due_to_expiry and (not exited_due_to_roll):
            continue
        reason = (
            "expired_or_untradable" if exited_due_to_expiry else "inside_roll_window"
        )
        events.append(
            {
