...
async def run_valuechain_preflight(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        report["chain_id"] = (
            int(str(chain_hex), 16)
            if isinstance(chain_hex, str) and chain_hex.startswith("0x")
            else None
        )
        report["ready"] = report["chain_id"] == expected
        if not report["ready"]:
            report["missing_or_wrong"] = (
                "ValueChain RPC did not return the documented chain ID"
            )
    ...
    print_status_line(
        f"ValueChain RPC {status}: chain_id={report.get('chain_id')} expected={expected} rpc={rpc_url}",
        style=style,
    )
